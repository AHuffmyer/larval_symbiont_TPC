...
neg_values<-neg_values%>%
  rename(peak=ID, file=File, group=Class, height=Height)%>%
  select(peak, file, group, height)%>%
  mutate(polarity=c("negative"))
```
...
negative<-negative%>%
  select(sample, temperature, parent, type, peak, height, polarity, protein.ug)
...
negative<-negative%>%
  ...
  summarise(height=sum(height, na.rm=TRUE))
```
...
standard_neg_value<-negative%>%
  ...
  group_by(sample)%>%
  summarise(height_standard=mean(height, na.rm=TRUE))
...
```{r}
negative<-negative%>%
   mutate(height_corrected=height/height_standard)
```

Finally, normalize corrected height to sample protein. This will generate units of height / ug protein. 

```{r}
negative<-negative%>%
  mutate(height_normalized=height_corrected/protein.ug)
```
...
The data frames now contain `peak` for a unique ID for each lipid detected. This number links the two files. Area represents the normalized height after internal standard normalization. We will further normalize this value below.  
...
# Filter lipids with both polarities to keep only the one with the highest height
...
lipids_filtered<-lipids_filtered%>%
  select(sample, lipid, temperature, parent, type, polarity, formula, ontology, height_normalized)%>%
  group_by(sample, lipid, temperature, parent, type, formula, ontology)%>%
  summarize(height_normalized=sum(height_normalized, na.rm=TRUE))
```
...
```{r}
hist(lipids_filtered$height_normalized)
```
...
wide_data<-lipids_filtered%>%
  ...
  select(!ontology)%>%
  select(!type)%>%
  pivot_wider(names_from=lipid, values_from=height_normalized)
```
...
test<-lipids_filtered%>%
  ...
  summarise(RSD=(sd(height_normalized)*100)/mean(height_normalized))
...
Keep lipids with RSD <40 in all treatments (RSD high right now, come back to this).  
```{r}
test <- test %>%
  group_by(lipid, parent, temperature) %>%
  filter(all(RSD < 40))
...
```{r}
write_csv(lipids_filtered, file="output/lipids_metabolites/lipids/processed_lipids_height.csv")
```
...
```{r}
scaled.pca<-prcomp(wide_data[c(4:871)], scale=TRUE, center=TRUE) 
...
parent              2     4019 0.09260 2.5316  0.035 *
temperature         2     3601 0.08298 2.2686  0.055 .
parent:temperature  4     2441 0.05624 0.7687  0.645  
Residual           42    33339 0.76817                
Total              50    43400 1.00000  
...
ggsave(pca_plots, filename="figures/lipids/pca_plots_height.jpeg", width=12, height=4)
```
...
ggsave(pca3, filename="figures/lipids/pca_all_groups_height.jpeg", width=8, height=6)
```
...
X<-X[4:871] #pull only data columns
...
pdf("figures/lipids/PLSDA_Parent_height.pdf", width=9, height=6)
...
write_csv(parent_VIP_1, "output/lipids_metabolites/lipids/Overall_Parent_VIPs_height.csv")
...
pdf("figures/lipids/Parent_VIP_height.pdf", width=10, height=35)
parent_VIP_1 %>%
  arrange(VIP) %>%
  ggplot( aes(x = VIP, y = reorder(Lipid,VIP,sum))) +
  geom_point() +
...
dev.off()
pdf("figures/lipids/VIP_heatmap_parent_height.pdf", width=4, height=6)
draw(heatmap)
dev.off()
```
...
```{r}
scaled.pca<-prcomp(vip_data_parent[c(4:252)], scale=TRUE, center=TRUE) 
...
```{r}
# scale data
vegan <- scale(vip_data_parent[c(4:252)])
...
adonis2(formula = vegan ~ parent * temperature, data = vip_data_parent, method = "eu")
                   Df SumOfSqs      R2      F Pr(>F)    
parent              2   2771.7 0.22262 7.1593  0.001 ***
temperature         2    972.3 0.07810 2.5115  0.032 *  
parent:temperature  4    576.1 0.04628 0.7441  0.702    
Residual           42   8129.9 0.65300                  
Total              50  12450.0 1.00000           
...
ggsave(pca_plots_parent, filename="figures/lipids/pca_plots_parentVIP_height.jpeg", width=12, height=4)
```
...
ggsave(pca_parent3, filename="figures/lipids/pca_all_groups_parentVIP_height.jpeg", width=8, height=6)
```
...
pdf("figures/lipids/PLSDA_Temperature_height.pdf", width=9, height=6)
...
write_csv(temperature_VIP_1, "output/lipids_metabolites/lipids/Overall_Temperature_VIPs_height.csv")
...
pdf("figures/lipids/Temperature_VIP_height.pdf", width=10, height=35)
temperature_VIP_1 %>%
  arrange(VIP) %>%
  ggplot( aes(x = VIP, y = reorder(Lipid,VIP,sum))) +
  geom_point() +
...
dev.off()
pdf("figures/lipids/VIP_heatmap_temperature_height.pdf", width=4, height=6)
draw(heatmap)
dev.off()
```
...
adonis2(formula = vegan ~ parent * temperature, data = vip_data_temperature, method = "eu")
                   Df SumOfSqs      R2      F Pr(>F)  
parent              2   1278.5 0.10187 2.9061  0.016 *
temperature         2   1312.5 0.10458 2.9833  0.011 *
parent:temperature  4    720.1 0.05738 0.8184  0.612  
Residual           42   9238.9 0.73616                
...
ggsave(pca_plots_temperature, filename="figures/lipids/pca_plots_temperatureVIP_height.jpeg", width=12, height=4)
```
...
ggsave(pca_temperature3, filename="figures/lipids/pca_all_groups_temperatureVIP_height.jpeg", width=8, height=6)
```
...
pdf("figures/lipids/PLSDA_interaction_height.pdf", width=9, height=6)
plotIndiv(MyResult.plsda, col=cols, ind.names = FALSE, legend=TRUE, legend.title = "Temperature", ellipse = FALSE, title="", style = "graphics", centroid=FALSE, point.lwd = 2, cex=2)
dev.off() 
...
write_csv(interaction_VIP_1, "output/lipids_metabolites/lipids/Overall_Interaction_VIPs_height.csv")
...
pdf("figures/lipids/Interaction_VIP_height.pdf", width=10, height=35)
interaction_VIP_1 %>%
  arrange(VIP) %>%
  ggplot( aes(x = VIP, y = reorder(Lipid,VIP,sum))) +
  geom_point() +
...
dev.off()
pdf("figures/lipids/VIP_heatmap_parentxtemperature_height.pdf", width=6, height=8)
draw(heatmap)
dev.off()
```
...
```{r}
scaled.pca<-prcomp(vip_data_interaction[c(4:308)], scale=TRUE, center=TRUE) 
...
```{r}
# scale data
vegan <- scale(vip_data_interaction[c(4:308)])
...
adonis2(formula = vegan ~ parent * temperature, data = vip_data_interaction, method = "eu")
                   Df SumOfSqs      R2      F Pr(>F)    
parent              2   2835.8 0.18595 6.0445  0.001 ***
temperature         2   1523.6 0.09991 3.2476  0.006 ** 
parent:temperature  4   1038.6 0.06810 1.1069  0.331    
Residual           42   9852.0 0.64604                  
Total              50  15250.0 1.00000                
...
ggsave(pca_plots_interaction, filename="figures/lipids/pca_plots_interactionVIP_height.jpeg", width=12, height=4)
```
...
ggsave(pca_interaction3, filename="figures/lipids/pca_all_groups_interactionVIP_height.jpeg", width=8, height=6)
```
...
parent_vip_list%>%
  as.data.frame()%>%
  write_csv(., file="output/lipids_metabolites/lipids/parent_VIP_list_height.csv")
```
...
temperature_vip_list%>%
  as.data.frame()%>%
  write_csv(., file="output/lipids_metabolites/lipids/temperature_VIP_list_height.csv")
```
...
interaction_vip_list%>%
  as.data.frame()%>%
  write_csv(., file="output/lipids_metabolites/lipids/interaction_VIP_list_height.csv")
```
...
full_lipid_list<-lipids_filtered%>%
  ...
  unique()%>%
  as.data.frame()%>%
  write_csv(., file="output/lipids_metabolites/lipids/full_lipid_list_height.csv")
```
...
pdf("figures/lipids/upset_VIP_list_overlap_height.pdf", width=6, height=6)
