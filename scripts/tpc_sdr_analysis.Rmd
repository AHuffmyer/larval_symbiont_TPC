...
r_plot1<-tpc_data %>%
    ...
    group_by(Temp_corrected, Symbiont, Date)%>%
    ...
    ggplot(., aes(x = Temp_corrected, y = mean, group=interaction(Temp_corrected, Symbiont))) +
    facet_wrap(~Date)+
    geom_hline(yintercept=0, linetype="dashed", color="black", size=0.75)+
    geom_point(aes(group=interaction(Temp_corrected, Symbiont), colour=Symbiont), size=6, position = position_dodge(0.4)) + 
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se, group=interaction(Temp_corrected, Symbiont)), width=0, linetype="solid", position=position_dodge(0.4), size=0.8, color="black")+
    xlab("Temperature") + 
...
r_plot2<-tpc_data %>%
  ...
  filter(!Temperature==28)%>%
    ...
    ggplot(., aes(x = Temp_corrected, y = (R.nmol.org.min*-1), group=Symbiont)) +
    facet_wrap(~Date)+
    geom_hline(yintercept=0, linetype="dashed", color="black", size=0.75)+
...
r_plot3<-tpc_data %>%
  ...
  filter(!Temperature==28)%>%
    ...
    ggplot(., aes(x = Temp_corrected, y = (R.nmol.org.min*-1), group=interaction(Temp_corrected, Symbiont))) +
    facet_wrap(~Date)+
    geom_hline(yintercept=0, linetype="dashed", color="black", size=0.75)+
    geom_point(aes(group=interaction(Temp_corrected, Symbiont), colour=Symbiont), size=4, position = position_jitterdodge(0.4)) + 
...
aov<-tpc_data%>%
    filter(!Temperature==28)%>%
    aov(R.nmol.org.min~Temp_corrected*Symbiont, data=.)

summary(aov)
```
No effect of symbiont, but significant effect of temperature. 
...
outlier.plot <- ggbetweenstats(tpc_data_filt, Temp_corrected, R.inverse, outlier.tagging = TRUE)
...
outlier.plot2 <- ggbetweenstats(tpc_data_outlier, Temperature, R.inverse, outlier.tagging = TRUE)
outlier.plot2

ggsave("figures/tpc_sdr/removed_outliersbygroup.png", outlier.plot2, dpi=300, w=16, h=8, units="in")
```
The outlier removal is not working well, conduct manual outlier removal. 

```{r}
tpc_data_filt<-tpc_data_filt%>%
  filter(!R.inverse>0.05)%>%
  filter(!R.inverse< -0.03)%>%
  filter(!Temperature==28)

outlier.plot2 <- ggbetweenstats(tpc_data_filt, Temperature, R.inverse, outlier.tagging = TRUE)
outlier.plot2
...
r_plot5<-tpc_data_filt %>%
    ...
    ggplot(., aes(x = as.factor(Temp_corrected), y = (R.nmol.org.min*-1), group=Symbiont)) +
    facet_wrap(~Date)+
    geom_hline(yintercept=0, linetype="dashed", color="black", size=0.75)+
...
```{r}
summary(aov(R.nmol.org.min~Temp_corrected*Symbiont, data=tpc_data_filt))
```
Significant effect of temperature, no effect of symbiont. 
...
# get start vals
start_vals <- get_start_vals(tpc_data_filt.All$Temp_corrected, tpc_data_filt.All$R.inverse, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(tpc_data_filt.All$Temp_corrected, tpc_data_filt.All$R.inverse, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(tpc_data_filt.All$Temp_corrected, tpc_data_filt.All$R.inverse, model_name = 'sharpeschoolhigh_1981')

fit <- nls_multstart(R.inverse~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref,e,eh,th, tref = 26),
...
# predict new data
new_data <- data.frame(temp = seq(min(tpc_data_filt.All$Temp_corrected), max(tpc_data_filt.All$Temp_corrected), by=((max(tpc_data_filt.All$Temp_corrected)-min(tpc_data_filt.All$Temp_corrected))/(nrow(tpc_data_filt.All)-1))))

preds <- augment(fit, newdata = new_data)

# plot data and model fit
ggplot(tpc_data_filt.All, aes(x=Temp_corrected, y=R.inverse)) +
  geom_point() +
...
```{r}
#refit 
fit_nlsLM <- minpack.lm::nlsLM(R.inverse~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref, e, eh, th, tref = 26),
                        data = tpc_data_filt.All,
                        start = coef(fit),
                        lower = low_lims,
                        upper = upper_lims,
...
boot_preds <- boot$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(tpc_data_filt.All$Temp_corrected), max(tpc_data_filt.All$Temp_corrected), length.out = 100))) %>%
  ungroup() %>%
...
CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp_corrected, R.inverse), tpc_data_filt.All, size = 2, alpha = 0.5,col = 'black') +
  theme_bw(base_size = 12) +
  theme_classic()+
  labs(x = 'Temperature (ºC)',
...
# get start vals
start_vals <- get_start_vals(tpc_data_filt.C$Temp_corrected, tpc_data_filt.C$R.inverse, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(tpc_data_filt.C$Temp_corrected, tpc_data_filt.C$R.inverse, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(tpc_data_filt.C$Temp_corrected, tpc_data_filt.C$R.inverse, model_name = 'sharpeschoolhigh_1981')

fit.C <- nls_multstart(R.inverse~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref,e,eh,th, tref = 26),
...
# predict new data
new_data.C <- data.frame(temp = seq(min(tpc_data_filt.C$Temp_corrected), max(tpc_data_filt.C$Temp_corrected), by=((max(tpc_data_filt.C$Temp_corrected)-min(tpc_data_filt.C$Temp_corrected))/(nrow(tpc_data_filt.C)-1))))

preds.C <- augment(fit.C, newdata = new_data.C)

# plot data and model fit
ggplot(tpc_data_filt.C, aes(x=Temp_corrected, y=R.inverse)) +
  geom_point() +
...
```{r}
#refit 
fit_nlsLM.C <- minpack.lm::nlsLM(R.inverse~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref, e, eh, th, tref = 26),
                        data = tpc_data_filt.C,
                        start = coef(fit.C),
                        lower = low_lims,
                        upper = upper_lims,
...
boot_preds.C <- boot.C$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(tpc_data_filt.C$Temp_corrected), max(tpc_data_filt.C$Temp_corrected), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = sharpeschoolhigh_1981(temp, r_tref, e, eh, th, tref = 26))
...
CI.plot.C <- ggplot() +
  ...
  geom_point(aes(Temp_corrected, R.inverse), tpc_data_filt.C, size = 2, alpha = 0.5,col = 'orange') +
  theme_bw(base_size = 12) +
    theme_classic()+
  labs(x = 'Temperature (ºC)',
       y = 'Respiration Rate (nmol O2/larva/min)', 
...
# get start vals
start_vals <- get_start_vals(tpc_data_filt.MIX$Temp_corrected, tpc_data_filt.MIX$R.inverse, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(tpc_data_filt.MIX$Temp_corrected, tpc_data_filt.MIX$R.inverse, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(tpc_data_filt.MIX$Temp_corrected, tpc_data_filt.MIX$R.inverse, model_name = 'sharpeschoolhigh_1981')

fit.MIX <- nls_multstart(R.inverse~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref,e,eh,th, tref = 26),
...
# predict new data
new_data.MIX <- data.frame(temp = seq(min(tpc_data_filt.MIX$Temp_corrected), max(tpc_data_filt.MIX$Temp_corrected), by=((max(tpc_data_filt.MIX$Temp_corrected)-min(tpc_data_filt.MIX$Temp_corrected))/(nrow(tpc_data_filt.MIX)-1))))

preds.MIX <- augment(fit.MIX, newdata = new_data.MIX)

# plot data and model fit
ggplot(tpc_data_filt.MIX, aes(x=Temp_corrected, y=R.inverse)) +
  geom_point() +
  geom_line(aes(temp, .fitted), preds.MIX, col = 'brown4') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Metabolic rate',
       title = 'Respiration across temperatures: MIX')
```
...
```{r}
#refit 
fit_nlsLM.MIX <- minpack.lm::nlsLM(R.inverse~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref, e, eh, th, tref = 26),
                        data = tpc_data_filt.MIX,
                        start = coef(fit.MIX),
                        lower = low_lims,
                        upper = upper_lims,
...
boot_preds.MIX <- boot.MIX$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(tpc_data_filt.MIX$Temp_corrected), max(tpc_data_filt.MIX$Temp_corrected), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = sharpeschoolhigh_1981(temp, r_tref, e, eh, th, tref = 26))
...
CI.plot.MIX <- ggplot() +
  ...
  geom_point(aes(Temp_corrected, R.inverse), tpc_data_filt.MIX, size = 2, alpha = 0.5,col = 'brown4') +
  theme_bw(base_size = 12) +
    theme_classic()+
  labs(x = 'Temperature (ºC)',
       y = 'Respiration Rate (nmol O2/larva/min)', 
...
# get start vals
start_vals <- get_start_vals(tpc_data_filt.WT$Temp_corrected, tpc_data_filt.WT$R.inverse, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(tpc_data_filt.WT$Temp_corrected, tpc_data_filt.WT$R.inverse, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(tpc_data_filt.WT$Temp_corrected, tpc_data_filt.WT$R.inverse, model_name = 'sharpeschoolhigh_1981')

fit.WT <- nls_multstart(R.inverse~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref,e,eh,th, tref = 26),
...
# predict new data
new_data.WT <- data.frame(temp = seq(min(tpc_data_filt.WT$Temp_corrected), max(tpc_data_filt.WT$Temp_corrected), by=((max(tpc_data_filt.WT$Temp_corrected)-min(tpc_data_filt.WT$Temp_corrected))/(nrow(tpc_data_filt.WT)-1))))

preds.WT <- augment(fit.WT, newdata = new_data.WT)

# plot data and model fit
ggplot(tpc_data_filt.WT, aes(x=Temp_corrected, y=R.inverse)) +
  geom_point() +
  geom_line(aes(temp, .fitted), preds.WT, col = 'gray') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Metabolic rate',
       title = 'Respiration across temperatures: WT')
```
...
```{r}
#refit 
fit_nlsLM.WT <- minpack.lm::nlsLM(R.inverse~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref, e, eh, th, tref = 26),
                        data = tpc_data_filt.WT,
                        start = coef(fit.WT),
                        lower = low_lims,
                        upper = upper_lims,
...
boot_preds.WT <- boot.WT$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(tpc_data_filt.WT$Temp_corrected), max(tpc_data_filt.WT$Temp_corrected), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = sharpeschoolhigh_1981(temp, r_tref, e, eh, th, tref = 26))
...
CI.plot.WT <- ggplot() +
  ...
  geom_point(aes(Temp_corrected, R.inverse), tpc_data_filt.MIX, size = 2, alpha = 0.5,col = 'gray') +
  theme_bw(base_size = 12) +
    theme_classic()+
  labs(x = 'Temperature (ºC)',
       y = 'Respiration Rate (nmol O2/larva/min)', 
...
# plot data and model fit
TPC.plot <- ggplot(data=tpc_data_filt.All, aes(x=Temp_corrected)) +
   
   geom_point(aes(x=Temp_corrected, y=R.inverse, color="C"), data=tpc_data_filt.C, size = 2, alpha = 0.5) +
   geom_point(aes(x=Temp_corrected, y=R.inverse, color="MIX"), data=tpc_data_filt.MIX, size = 2, alpha = 0.5) +
   geom_point(aes(x=Temp_corrected, y=R.inverse, color="WT"), data=tpc_data_filt.WT, size = 2, alpha = 0.5) +
   ...
   #xlim(21,33)+
   scale_x_continuous(breaks=c(13,23,26,28,32,35,37))+
   theme_bw(base_size = 12) +
   theme_classic()+
...
All_params%>%
  filter(param=="eh")
```
Eh is the decrease in activity at high temperatures. This value is lower in Mixed larvae than Cladocopium larvae. This suggests a higher loss of energy at high temperatures in Cladocopium larvae. 

There are some parameters that were not estimated from Mixed, we will need to revisit that curve. 
...
ggsave("figures/tpc_sdr/TPC_estimates_all_symbionts.png", estimate.plots, dpi=300, w=8, h=8, units="in")
```
