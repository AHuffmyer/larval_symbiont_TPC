...
r_plot1<-tpc_data %>%
    ...
    ggplot(., aes(x = as.factor(Temperature), y = abs(mean), group=interaction(Temperature, Symbiont))) +
    facet_wrap(~Date)+
...
r_plot2<-tpc_data %>%
    ...
    ggplot(., aes(x = as.factor(Temperature), y = abs(R.nmol.org.min), group=Symbiont)) +
    facet_wrap(~Date)+
    geom_hline(yintercept=0, linetype="dashed", color="black", size=0.75)+
...
outlier.plot <- ggbetweenstats(tpc_data, Temperature, R.nmol.org.min, outlier.tagging = TRUE)
outlier.plot
...
outlier.plot2 <- ggbetweenstats(tpc_data, Temperature, R.nmol.org.min, outlier.tagging = TRUE)
outlier.plot2
