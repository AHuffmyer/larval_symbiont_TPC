...
r_plot2<-tpc_data %>%
  ...
  filter(!Temperature==40)%>%
    ...
    ggplot(., aes(x = Temp.C, y = (R.nmol.org.min*-1), group=Symbiont)) +
    facet_wrap(~Date)+
    geom_hline(yintercept=0, linetype="dashed", color="black", size=0.75)+
...
r_plot3<-tpc_data %>%
  ...
  filter(!Temperature==40)%>%
    ...
    ggplot(., aes(x = Temp.C, y = (R.nmol.org.min*-1), group=interaction(Temp.C, Symbiont))) +
    facet_wrap(~Date)+
    geom_hline(yintercept=0, linetype="dashed", color="black", size=0.75)+
...
aov<-tpc_data%>%
   ...
   filter(!Temperature==39)%>%
   filter(!Temperature==40)%>%
    aov(R.nmol.org.min~Temp.C*Symbiont, data=.)

summary(aov)
```

Effect of temp and symbiont 
...
```{r}
summary(aov(R.nmol.org.min~Temp.C*Symbiont, data=tpc_data_filt))
```
Significant effect of temperature and symbiont  
...
NOW RUN THESE WITH NEW FILTERED DATA - REVISIT WHICH TEMPERATURES WE INCLUDE OR DONT 
