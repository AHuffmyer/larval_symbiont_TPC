...
The outlier removal is not accurate, keep all data points for now. 
...
# get start vals
start_vals <- get_start_vals(as.numeric(tpc_data_filt$Temp_corrected),tpc_data_filt$R.inverse, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(as.numeric(tpc_data_filt$Temp_corrected),tpc_data_filt$R.inverse, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(as.numeric(tpc_data_filt$Temp_corrected),tpc_data_filt$R.inverse, model_name = 'sharpeschoolhigh_1981')
...
NEED TO CORRECT NA IN START VALUES 
...
```{r}
# CURVE FIT 
C_data<-tpc_data_filt%>%
  filter(Symbiont=="C") #%>%
  #mutate(R.inverse=as.numeric(R.inverse))%>%
  #mutate(Temp_corrected=as.numeric(as.character(Temperature)))

#fit 
C.fit <- nls_multstart(R.inverse~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref, e, eh, th, tref = 26),
                                                     data = C_data,
                                                     iter = 500,
...
C.new_data <- data.frame(temp = seq(from=min(C_data$Temp_corrected), to=max(C_data$Temp_corrected), by=((max(C_data$Temp_corrected)-min(C_data$Temp_corrected))/(nrow(C_data)-1)))) #set min and max temperatures and add prediction for number of rows in data frame 

#C.new_data <- data.frame(temp = seq(from=min(C_data$Temp_corrected), to=max(C_data$Temp_corrected), by=0.071)) #manual interval if required
...
C.fit_nlsLM <- minpack.lm::nlsLM(R.nmol.org.min~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref,e,eh,th, tref = 28),
                        data = C_data,
                        start = coef(C.fit),
                        lower = low_lims,
                        upper = upper_lims,
...
C.boot1_preds <- C.boot1$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(C_data$Temp_corrected), max(C_data$Temp_corrected), length.out = 100))) %>%
  ungroup() %>%
...
C.CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp_corrected, R.inverse), C_data, size = 2, alpha = 0.5,col = 'orange') +
...
Run a model fit for Mixed symbionts 
```{r}
# CURVE FIT 
MIX_data<-tpc_data_filt%>%
  filter(Symbiont=="MIX")

#fit 
MIX.fit <- nls_multstart(R.nmol.org.min~sharpeschoolhigh_1981(temp = Temp_corrected, r_tref,e,eh,th, tref = 28),
                                                     data = MIX_data,
                                                     iter = 500,
                                                     start_lower = start_vals - 1,
                                                     start_upper = start_vals + 1,
