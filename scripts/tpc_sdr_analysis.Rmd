...
outlier.plot <- ggbetweenstats(tpc_data_filt, Temp.C, R.inverse, outlier.tagging = TRUE)
...
outlier.plot2 <- ggbetweenstats(tpc_data_outlier, Temp.C, R.inverse, outlier.tagging = TRUE)
outlier.plot2
...
outlier.plot2 <- ggbetweenstats(tpc_data_filt, Temp.C, R.inverse, outlier.tagging = TRUE)
outlier.plot2
...
r_plot5<-tpc_data_filt %>%
    ...
    ggplot(., aes(x = as.numeric(Temp.C), y = (R.nmol.org.min*-1), group=Symbiont)) +
    facet_wrap(~Date)+
    geom_hline(yintercept=0, linetype="dashed", color="black", size=0.75)+
...
```
Effect of symbiont and temperature. 
...
# get start vals
start_vals <- get_start_vals(tpc_data_filt.All$Temp.C, tpc_data_filt.All$R.inverse, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(tpc_data_filt.All$Temp.C, tpc_data_filt.All$R.inverse, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(tpc_data_filt.All$Temp.C, tpc_data_filt.All$R.inverse, model_name = 'sharpeschoolhigh_1981')

fit <- nls_multstart(R.inverse~sharpeschoolhigh_1981(temp = Temp.C, r_tref,e,eh,th, tref = 26),
...
ggplot(tpc_data_filt.All, aes(x=Temp.C, y=R.inverse)) +
  geom_point() +
...
```{r}
#refit 
fit_nlsLM <- minpack.lm::nlsLM(R.inverse~sharpeschoolhigh_1981(temp = Temp.C, r_tref, e, eh, th, tref = 26),
                        data = tpc_data_filt.All,
                        start = coef(fit),
                        lower = low_lims,
                        upper = upper_lims,
...
CI.plot <- ggplot() +
  ...
  geom_point(aes(Temp.C, R.inverse), tpc_data_filt.All, size = 2, alpha = 0.5,col = 'black') +
  theme_bw(base_size = 12) +
  theme_classic()+
  labs(x = 'Temperature (ºC)',
...
# get start vals
start_vals <- get_start_vals(tpc_data_filt.C$Temp.C, tpc_data_filt.C$R.inverse, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(tpc_data_filt.C$Temp.C, tpc_data_filt.C$R.inverse, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(tpc_data_filt.C$Temp.C, tpc_data_filt.C$R.inverse, model_name = 'sharpeschoolhigh_1981')

fit.C <- nls_multstart(R.inverse~sharpeschoolhigh_1981(temp = Temp.C, r_tref,e,eh,th, tref = 26),
...
# plot data and model fit
ggplot(tpc_data_filt.C, aes(x=Temp.C, y=R.inverse)) +
  geom_point() +
...
```{r}
#refit 
fit_nlsLM.C <- minpack.lm::nlsLM(R.inverse~sharpeschoolhigh_1981(temp = Temp.C, r_tref, e, eh, th, tref = 26),
                        data = tpc_data_filt.C,
                        start = coef(fit.C),
                        lower = low_lims,
                        upper = upper_lims,
...
CI.plot.C <- ggplot() +
  ...
  geom_point(aes(Temp.C, R.inverse), tpc_data_filt.C, size = 2, alpha = 0.5,col = 'orange') +
  theme_bw(base_size = 12) +
    theme_classic()+
  labs(x = 'Temperature (ºC)',
       y = 'Respiration Rate (nmol O2/larva/min)', 
...
# get start vals
start_vals <- get_start_vals(tpc_data_filt.MIX$Temp.C, tpc_data_filt.MIX$R.inverse, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(tpc_data_filt.MIX$Temp.C, tpc_data_filt.MIX$R.inverse, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(tpc_data_filt.MIX$Temp.C, tpc_data_filt.MIX$R.inverse, model_name = 'sharpeschoolhigh_1981')

fit.MIX <- nls_multstart(R.inverse~sharpeschoolhigh_1981(temp = Temp.C, r_tref,e,eh,th, tref = 26),
...
# plot data and model fit
ggplot(tpc_data_filt.MIX, aes(x=Temp.C, y=R.inverse)) +
  geom_point() +
...
```{r}
#refit 
fit_nlsLM.MIX <- minpack.lm::nlsLM(R.inverse~sharpeschoolhigh_1981(temp = Temp.C, r_tref, e, eh, th, tref = 26),
                        data = tpc_data_filt.MIX,
                        start = coef(fit.MIX),
                        lower = low_lims,
                        upper = upper_lims,
...
CI.plot.MIX <- ggplot() +
  ...
  geom_point(aes(Temp.C, R.inverse), tpc_data_filt.MIX, size = 2, alpha = 0.5,col = 'brown4') +
  theme_bw(base_size = 12) +
    theme_classic()+
  labs(x = 'Temperature (ºC)',
       y = 'Respiration Rate (nmol O2/larva/min)', 
...
# get start vals
start_vals <- get_start_vals(tpc_data_filt.WT$Temp.C, tpc_data_filt.WT$R.inverse, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(tpc_data_filt.WT$Temp.C, tpc_data_filt.WT$R.inverse, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(tpc_data_filt.WT$Temp.C, tpc_data_filt.WT$R.inverse, model_name = 'sharpeschoolhigh_1981')

fit.WT <- nls_multstart(R.inverse~sharpeschoolhigh_1981(temp = Temp.C, r_tref,e,eh,th, tref = 26),
...
# plot data and model fit
ggplot(tpc_data_filt.WT, aes(x=Temp.C, y=R.inverse)) +
  geom_point() +
...
```{r}
#refit 
fit_nlsLM.WT <- minpack.lm::nlsLM(R.inverse~sharpeschoolhigh_1981(temp = Temp.C, r_tref, e, eh, th, tref = 26),
                        data = tpc_data_filt.WT,
                        start = coef(fit.WT),
                        lower = low_lims,
                        upper = upper_lims,
...
CI.plot.WT <- ggplot() +
  ...
  geom_point(aes(Temp.C, R.inverse), tpc_data_filt.MIX, size = 2, alpha = 0.5,col = 'gray') +
  theme_bw(base_size = 12) +
    theme_classic()+
  labs(x = 'Temperature (ºC)',
       y = 'Respiration Rate (nmol O2/larva/min)', 
...
TPC.plot <- ggplot(data=tpc_data_filt.All, aes(x=Temp.C)) +
   ...
   geom_point(aes(x=Temp.C, y=R.inverse, color="C"), data=tpc_data_filt.C, size = 2, alpha = 0.5) +
   geom_point(aes(x=Temp.C, y=R.inverse, color="MIX"), data=tpc_data_filt.MIX, size = 2, alpha = 0.5) +
...
All_params%>%
  filter(param=="ctmax")
```
T opt is higher in C larvae than D larvae in the dark/absence of photosynthesis (slightly overlapping intervals though). 
Thermal safety margin is higher in MIX than C. 
CTmax is higher in MIX than in C. 

Lower thermal safety margin means that C larvae are closer to their thermal maximum in conditions they are normally in. MIX has greater distance between maximum and normal conditions. MIX larvae have a higher upper thermal limit that C larvae. 

This is interesting! C larvae have a higher optimal temperature, but lower CTmax and thermal safety margin. 
