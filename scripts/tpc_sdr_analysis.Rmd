...
```{r}
summary(aov(R.nmol.org.min~Temperature*Symbiont, data=tpc_data))
```
...
```{r}
summary(aov(R.nmol.org.min~Temperature*Symbiont, data=tpc_data_outlier))
```
...
# get start vals
start_vals <- get_start_vals(as.numeric(tpc_data_outlier$Temperature),tpc_data_outlier$R.nmol.org.min, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(as.numeric(tpc_data_outlier$Temperature),tpc_data_outlier$R.nmol.org.min, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(as.numeric(tpc_data_outlier$Temperature),tpc_data_outlier$R.nmol.org.min, model_name = 'sharpeschoolhigh_1981')
...
```{r}
# CURVE FIT 
C_data<-tpc_data_outlier%>%
  filter(Symbiont=="C")%>%
  mutate(R.inverse=as.numeric(R.inverse))%>%
  mutate(Temperature=as.numeric(as.character(Temperature)))

#fit 
C.fit <- nls_multstart(R.inverse~sharpeschoolhigh_1981(temp = Temperature, r_tref, e, eh, th, tref = 26),
                                                     data = C_data,
                                                     iter = 500) #,
                                                     start_lower=c(r_tref=0.2, e=2, eh=10, th=2),
                                                     start_upper=c(r_tref=0.3, e=10, eh=20, th=3), 
                                                     lower=c(r_tref=0.01, e=1, eh=5, th=1))
