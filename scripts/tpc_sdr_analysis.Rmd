...
The outlier removal is not accurate, keep all data points for now removing strong outliers. 

```{r}
tpc_data_filt<-tpc_data%>%
  filter(R.inverse>-0.035)%>%
...
## Plot data again after outlier removal 
...
ggsave("figures/tpc_sdr/tpc_meanss_outlier_removed.png", r_plot1, dpi=300, w=7, h=5, units="in")
```
...
ggsave("figures/tpc_sdr/tpc_loesss_outlier_removed.png", r_plot2, dpi=300, w=7, h=5, units="in")
```

Plot data with means   
```{r}
r_plot6<-tpc_data_filt %>%
  #filter(R.nmol.org.min<0.01)%>%
    
    ggplot(., aes(x = Temp_corrected, y = (R.nmol.org.min*-1), group=interaction(Temperature, Symbiont))) +
    facet_wrap(~Date)+
    geom_hline(yintercept=0, linetype="dashed", color="black", size=0.75)+
    geom_point(aes(group=interaction(Temp_corrected, Symbiont), colour=Symbiont), size=4, position = position_jitterdodge(0.4)) + 
    #geom_errorbar(aes(ymin=mean-se, ymax=mean+se, group=interaction(Temperature, Symbiont)), width=0, linetype="solid", position=position_dodge(0.4), size=0.8, color="black")+
...
ggsave("figures/tpc_sdr/tpc_dots_outlier_removed.png", r_plot3, dpi=300, w=7, h=5, units="in")
```
...
# get start vals
start_vals <- get_start_vals(as.numeric(tpc_data_filt$Temperature),tpc_data_filt$R.nmol.org.min, model_name = 'sharpeschoolhigh_1981')

# get limits
low_lims <- get_lower_lims(as.numeric(tpc_data_filt$Temperature),tpc_data_filt$R.nmol.org.min, model_name = 'sharpeschoolhigh_1981')
upper_lims <- get_upper_lims(as.numeric(tpc_data_filt$Temperature),tpc_data_filt$R.nmol.org.min, model_name = 'sharpeschoolhigh_1981')
...
MODELS AREN'T WORKING NOW, ISSUE WITH STARTING PARAMETERS 
...
```{r}
# CURVE FIT 
all_data<-tpc_data_filt%>%
  #filter(Symbiont=="C")%>%
  mutate(R.inverse=as.numeric(R.inverse))%>%
  mutate(Temperature=as.numeric(as.character(Temperature)))

#fit 
C.fit <- nls_multstart(R.nmol.org.min~sharpeschoolhigh_1981(temp = Temperature, r_tref, e, eh, th, tref = 26),
                                                     data = C_data,
                                                     iter = 500, #,
                                                     start_lower=c(r_tref=-0.01, e=0, eh=0.15, th=3.6),
                                                     lower=c(r_tref=0.00, e=0, eh=0, th=1),
                                                     upper=c(r_tref=0.036, e=10, eh=20, th=6))
...
TPC.plot <- ggplot(data=tpc_data_outlier, aes(x=Temperature)) +
