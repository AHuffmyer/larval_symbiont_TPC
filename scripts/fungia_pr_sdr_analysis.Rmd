...
```{r}
r_data<-r_data%>%
  mutate(R.nmol.org.min=log10(1+R.nmol.org.min))
...
```{r}
r_data<-r_data%>%filter(R.nmol.org.min < 0.003)
boxplot(r_data$R.nmol.org.min)
```
Removed one outlier 
...
AH recommends using 10 larvae per well. This will make counting easier and will prevent oxygen depletion that likely drives reduced respiration rates at >12 larvae. 
