...
It is interesting that LER is lower than Rd. We would expect LER to be higher due to stimulation of host respiration through photosynthesis. I suspect that this is lower than Rd in our case because the larvae are just coming out of a high light period (500 par) and therefore this stress may reduce respiration in subsequent darkness. 
...
TukeyHSD(model, c("Temperature"))
```
LER is affected by temperature.
...
TukeyHSD(model, c("Temperature"))
```
Rd is affected by temperature and symbiont. WT and MIX are different, WT is higher than MIX. No difference between MIX and C or WT and C (almost sign). There is higher dark respiration in WT. Dark respiration increases with temp then crashes at 36C. 
...
pairs(emm)
```
There are light and temperature and symbiont effects on P:R ratio. There is a symbiont x temperature effect as well. Only at 27C there are higher P:R ratios in MIX larvae than C larvae (p=0.005) particularly at medium light (p=0.005) and slightly at high light (p=0.08)! Overall, MIX is slightly higher than WT for P:R ratios (p=0.057). 
