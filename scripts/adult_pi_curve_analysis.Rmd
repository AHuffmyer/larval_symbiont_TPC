...
## Import data
```{r}
path.p <- "data/adult_pi_curves/" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "data/adult_pi_curves/pi_curves_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/adult_pi_curves/pi_curves_run_metadata.csv")

# Join all coral and run metadata
metadata <- full_join(sample.info, run.info) %>%
  mutate(Date = as_date(as.character(Date), format = "%Y%m%d", tz = "Pacific/Honolulu"))
