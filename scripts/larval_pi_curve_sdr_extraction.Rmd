...
```{r}
Run.Info<-read.csv("data/larval_pi_curves/larval_PIcurve_SDR_Run_Info.csv")%>%
...
```{r}
# --- paths ---
input_dir  <- "data/larval_pi_curves/runs"
output_dir <- "output/larval_pi_curves/PhotosynthesisPlots"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

# --- files ---
files <- list.files(input_dir, pattern = "\\.csv$", full.names = TRUE)
files <- files[!grepl("20230621", files)]   # omit test data from files to be read in as data
...
readr::write_csv(summary_df, file.path("output/larval_pi_curves/photosynthesis_slope_summary_by_light_level.csv"))
```
...
readr::write_csv(temp_summary_plate, file.path("output/larval_pi_curves/mean_temperature_by_plate_and_light_interval.csv"))
```
...
Save as .csv file.  
```{r, warning=FALSE, message=FALSE}
write.csv(oxygen.bio, paste0("output/larval_pi_curves/PIcurves_calculated_normalized_photo_rates.csv")) #save final file
```