...
Reorder factor. 
```{r}
lipids_filtered$temperature<-factor(lipids_filtered$temperature, levels=c("Ambient", "Moderate (+3)", "High (+6)"))
```
...
# Proportion oxidation in PC lipids

```{r}
ox_pc<-lipids_filtered %>%
  mutate(class = str_extract(lipid, "^[A-Za-z]+"))%>%
  filter(class=="PC")%>%
  
  #add column for oxidized vs not oxidized 
  mutate(
    ox = if_else(
      str_detect(lipid, "O-"),
      "oxidized",
...
Summarize total concentration of oxidized vs non oxidized PC lipids. 

```{r}
ox_pc<-ox_pc%>%
  group_by(sample, temperature, parent, ox)%>%
  summarise(total_area=sum(area_normalized))
```
...
pc_plot1<-ox_pc%>%
  ...
  ggplot(aes(x = temperature, y = total_area, colour = parent)) +
  geom_boxplot()+
  geom_point(position=position_dodge(0.75)) +
  scale_colour_manual(values=c("orange", "darkred", "darkgray"))+
  labs(x = "", y = "Oxidized PC Lipids") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        legend.position="right");pc_plot1
```

Calculate oxidized:non-oxidized ratio. 

```{r}
ox_pc2<-ox_pc%>%
  pivot_wider(names_from = ox, values_from = total_area)%>%
  mutate(ratio = oxidized/`not oxidized`)
```
...
pc_plot2<-ox_pc2%>%
  ...
  geom_point(position=position_dodge(0.75)) +
  scale_colour_manual(values=c("orange", "darkred", "darkgray"))+
  labs(x = "", y = "Oxidized:Non-Oxidized PC Lipids") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        legend.position="right");pc_plot2
```


# Proportion oxidation in FA lipids

```{r}
ox_fa<-lipids_filtered %>%
  mutate(class = str_extract(lipid, "^[A-Za-z]+"))%>%
  filter(class=="FA")%>%
...
Summarize total concentration of oxidized vs non oxidized FA lipids. 
...
fa_plot2<-ox_fa2%>%
  ...
  geom_point(position=position_dodge(0.75)) +
  scale_colour_manual(values=c("orange", "darkred", "darkgray"))+
  labs(x = "", y = "Oxidized:Non-Oxidized FA Lipids") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        legend.position="right");fa_plot2
```