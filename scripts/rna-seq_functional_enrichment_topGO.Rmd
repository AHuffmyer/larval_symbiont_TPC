...
Test set of genes:  
```{r}
parent_test_set<-parent_DEG_list%>%rename("gene_id"=gene)%>%pull(gene_id)
...
The reduced list of terms is 148 terms that falls under 46 parent terms.
...
resultFisher
hist(score(resultFisher), 50, xlab = "p-values")
#35 terms with p<0.01

resultWeight <- runTest(GOdata, algorithm = "weight01", statistic = "fisher")
resultWeight
hist(score(resultWeight), 50, xlab = "p-values")
#21 terms with p<0.01
```
