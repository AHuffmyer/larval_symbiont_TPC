...
The reduced list of terms is 76 terms that falls under 32 parent terms.
...
ggsave(parent_enrich_plot3, filename="figures/rna_seq/topGO/parent_enrichment_individual_terms.png", width=10, height=11)
...
ggsave(parent_enrich_plot4, filename="figures/rna_seq/topGO/parent_enrichment_parent_terms.png", width=10, height=7)
```

Try subsetting anything related to lipid or fatty acid metabolism/activity. 

```{r}
parent_enrich_plot5 <- p05_res_reduced%>%
  filter(str_detect(Term, regex("lipid|fatty acid", ignore_case = TRUE)))%>%
  ...
  theme(axis.text.y = element_text(size=8),
        axis.title = element_text(size=9),
        plot.title = element_text(hjust=0.5, size=10));parent_enrich_plot5

ggsave(parent_enrich_plot5, filename="figures/rna_seq/topGO/parent_lipid_terms.png", width=7, height=5)
```
...
resultFisher
hist(score(resultFisher), 50, xlab = "p-values")
#358 terms with p<0.01

resultWeight <- runTest(GOdata, algorithm = "weight01", statistic = "fisher")
resultWeight
hist(score(resultWeight), 50, xlab = "p-values")
#118 terms with p<0.01
```
...
The reduced list of terms is 267 terms that falls under 55 parent terms.
...
temp_enrich_plot3 <- ggplot(p05_res_reduced, 
       aes(x = GeneRatio, 
           y = reorder(Term, GeneRatio), 
           color = -log10(weightFisher))) +
  geom_point(size=1) +
  ...
  theme(axis.text.y = element_text(size=8),
        axis.title = element_text(size=9),
        plot.title = element_text(hjust=0.5, size=10));temp_enrich_plot3

ggsave(temp_enrich_plot3, filename="figures/rna_seq/topGO/temperature_enrichment_individual_terms.png", width=10, height=25)
...
temp_enrich_plot4<-ggplot(parent_summary, aes(x = meanGeneRatio, y = reorder(ParentTerm, meanGeneRatio), color = maxLogP, size = nTerms)) +
  geom_point(size=2) +
  scale_color_gradient(low = "lightblue", high = "darkblue", name="Max -log10(weightFisher)") +
  scale_size_continuous(name="Number of Terms") +
  theme_classic(base_size=10) +
  labs(x="Mean Gene Ratio", y="Parent GO Term");temp_enrich_plot4

ggsave(temp_enrich_plot4, filename="figures/rna_seq/topGO/temperature_enrichment_parent_terms.png", width=10, height=7)
```

Try subsetting anything related to lipid or fatty acid metabolism/activity. 

```{r}
temp_enrich_plot5 <- p05_res_reduced%>%
  filter(str_detect(Term, regex("lipid|fatty acid", ignore_case = TRUE)))%>%
  ...
  theme(axis.text.y = element_text(size=8),
        axis.title = element_text(size=9),
        plot.title = element_text(hjust=0.5, size=10));temp_enrich_plot5

ggsave(temp_enrich_plot5, filename="figures/rna_seq/topGO/temperature_lipid_terms.png", width=7, height=6)
```

## Interaction enrichment: NB vs B at elevated temperature  
...
resultFisher
hist(score(resultFisher), 50, xlab = "p-values")
#177 terms with p<0.05

resultWeight <- runTest(GOdata, algorithm = "weight01", statistic = "fisher")
resultWeight
hist(score(resultWeight), 50, xlab = "p-values")
#44 terms with p<0.05
```
...
The reduced list of terms is 121 terms that falls under 42 parent terms.
...
interaction_enrich_plot3 <- ggplot(p05_res_reduced, 
       aes(x = GeneRatio, 
           y = reorder(Term, GeneRatio), 
           color = -log10(weightFisher))) +
  geom_point(size=1) +
  ...
  theme(axis.text.y = element_text(size=8),
        axis.title = element_text(size=9),
        plot.title = element_text(hjust=0.5, size=10));interaction_enrich_plot3

ggsave(interaction_enrich_plot3, filename="figures/rna_seq/topGO/interaction_enrichment_individual_terms.png", width=10, height=12)
...
interaction_enrich_plot4<-ggplot(parent_summary, aes(x = meanGeneRatio, y = reorder(ParentTerm, meanGeneRatio), color = maxLogP, size = nTerms)) +
  geom_point(size=2) +
  scale_color_gradient(low = "lightblue", high = "darkblue", name="Max -log10(weightFisher)") +
  scale_size_continuous(name="Number of Terms") +
  theme_classic(base_size=10) +
  labs(x="Mean Gene Ratio", y="Parent GO Term");interaction_enrich_plot4

ggsave(interaction_enrich_plot4, filename="figures/rna_seq/topGO/interaction_enrichment_parent_terms.png", width=10, height=8)
```
Gene Ratio: 

Proportion of the term represented in your DEGs:

- A gene ratio of 0.5 means half of all genes associated with that GO term are differentially expressed.

- A higher gene ratio usually indicates that this biological process is strongly affected in your experiment.

Relative enrichment within the GO term:

- Even if a GO term has a large p-value due to many genes, a high gene ratio signals that a large fraction of the genes in that category are responding.

- Conversely, a term with low gene ratio but very low p-value might be significant only because the category has many genes; fewer are actually changing.

Helps prioritize meaningful GO terms:

- You can focus on terms with both strong statistical significance (low p-value) and high gene ratio — these are likely biologically impactful.

Try subsetting anything related to lipid or fatty acid metabolism/activity. 

```{r}
interaction_enrich_plot5 <- p05_res_reduced%>%
  filter(str_detect(Term, regex("lipid|fatty acid", ignore_case = TRUE)))%>%
  ...
  theme(axis.text.y = element_text(size=8),
        axis.title = element_text(size=9),
        plot.title = element_text(hjust=0.5, size=10));interaction_enrich_plot5

ggsave(interaction_enrich_plot5, filename="figures/rna_seq/topGO/interaction_lipid_terms.png", width=7, height=5)
```
