...
# Plot and analyze: 20230626 

Plot data with means   
```{r}
pr_plot1<-pr_data %>%
    filter(Date=="20230626")%>%
    group_by(PAR, Symbiont, Temperature)%>%
    dplyr::summarise(mean=mean(P.nmol.org.min, na.rm=TRUE), sd=sd(P.nmol.org.min, na.rm=TRUE), N=length(P.nmol.org.min), se=sd/sqrt(N))%>%
...
ggsave("figures/pr_sdr/pr_means_0626.png", pr_plot1, dpi=300, w=7, h=5, units="in")
```
...
ggsave("figures/pr_sdr/pr_dots_0626.png", pr_plot3, dpi=300, w=7, h=5, units="in")
```
...
ggsave("figures/pr_sdr/pr_metrics_0624.png", metric_plot1, dpi=300, w=7, h=5, units="in")
```
...
If we remove the outlier in WT at 30C, there is a difference by symbiont effect of PR at high light. C is lower than WT at high light.  
```{r}
model<-pr_data_calc%>%
  filter(Date=="20230624")%>%
  filter(Metric=="ratio_high")%>%
...
# Calculate R, P, and P:R: 20230626 
...
ggsave("figures/pr_sdr/pr_metrics_0626.png", metric_plot1, dpi=300, w=7, h=5, units="in")
```
...
ggsave("figures/pr_sdr/pr_ratio_0626.png", metric_plot2, dpi=300, w=7, h=5, units="in")
```
