...
pi_data<-pi_data%>%filter(P.nmol.org.min > -0.06)%>%filter(P.nmol.org.min < 0.06)
boxplot(pi_data$P.nmol.org.min)
...
Filter out high light runs (inconsistent temprature and O2 readings)
```{r}
#pi_data<-pi_data%>%
  #filter(PAR<1100)%>%
  #filter(P.nmol.org.min>-0.03)
```
...
pairs(emm)
```
PAR and symbiont are significant, there is an interaction between PAR and symbiont. 

0 PAR = no difference 
16 PAR = no difference 
60 PAR = C is higher than MIX, C greater than WT 
120 PAR = no difference 
275 PAR = MIX less than WT 
425 PAR = C greater than WT, C greater than MIX
700 PAR = C greater than MIX, MIX less than WT 
1100 PAR = no difference
1500 PAR = C less than WT  
1660 PAR = no difference
...
metrics_plot1<-pi_results%>%
  pivot_longer(values_to="Value", names_to="Parameter", cols=c(Am:Ik))%>%
  filter(Parameter=="Ik")%>%
...
metrics_plot<-plot_grid(metrics_plot1, metrics_plot1a, nrow=1, ncol=2, rel_widths=c(0.6,1))

ggsave("figures/larval_pi_curves/parameters.png", metrics_plot, width=8, height=6, dpi=300)
```
