...
test<-test%>%
  mutate(
    ...
    AQY = ifelse(AQY > 0, AQY, NA),
    Rd = ifelse(Rd > 0, Rd, NA),
...
metrics_plot2<-test%>%
  ...
  ggplot(aes(x=Symbiont, y=value, color=Symbiont))+
    ...
    scale_color_manual(name="Symbiont", values=c("orange","brown4", "gray"), labels=parent_names)+
    theme_classic()+
  ggtitle("B")+
...
curves_plot<-ggplot(summary_curves, aes(x = Light_Value, y = fit_mean, color = Symbiont, fill = Symbiont)) +
  ...
  theme_classic() +
  ylim(-0.005, 0.01) +
    scale_color_manual(name="Parent", values = c("orange", "brown4", "gray"), labels=parent_names) +
  scale_fill_manual(name="Parent", values = c("orange", "brown4", "gray"), labels=parent_names) +
