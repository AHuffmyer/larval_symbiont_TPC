...
There are 1320 DEGs between NB and B at 30°C. There are 3022 at 33°C. In these contrasts, positive fold changes indicate higher expression in the first listed contrast - in this case, Nonbleached samples.  
...
```
There are 4342 total genes and 3599 unique genes, most are shared between the 30°C and 33°C temperature contrasts.   
...
```
There are 3559 genes that are uniquely differentially expressed between NB and B at elevated temperature (30 or 33°C). 
...
scaled_test_DEG_interaction <-prcomp(test_DEG_interaction[c(1:3526)], scale=TRUE, center=TRUE)
...
adonis2(formula = vegan_DEG_interaction ~ parent * temperature, data = test_DEG_interaction, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2   2308.0 0.43116 26.2143  0.001 ***
temperature         2    779.6 0.14563  8.8543  0.001 ***
parent:temperature  4    284.5 0.05315  1.6158  0.030 *  
Residual           45   1981.0 0.37007                   
...
DEG_PCA_interaction <- ggplot(gPCAdata_DEG_interaction, aes(PC1, PC2, color=parent, shape=temperature)) + 
  geom_point(size=3) + 
  ggtitle(expression(bold("DEGs: NB vs B at 30-33°C (3526 genes)")))+
  xlab(paste0("PC1: ",percentVar_DEG_parent[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar_DEG_parent[2],"% variance")) +
  scale_color_manual(name="Parental Phenotype", values=c("Wildtype"="gray", "Bleached"="orange", "Nonbleached"="brown4"))+
  scale_shape_manual(name="Temperature", values=c(19,17,15))+
