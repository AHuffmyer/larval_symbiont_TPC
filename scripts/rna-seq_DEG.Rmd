...
Next steps: Add an analysis using pairwise group model with Wald test to see if DEGs are the same as those detected with interactive effect model. Look for DEGs at 33°C between B and NB that are not present at 27°C. 
...
There are 24 genes differentially expressed between 27 and 30°C. There are 914 genes differentially expressed between 27 and 33°C. There are 224 genes differentially expressed between 30 and 33°C. In these contrasts, positive fold changes indicate higher expression in the first listed contrast.  
...
parent:temperature  4     1822 0.03672  1.3958  0.172    
...
DEG_PCA_parent <- ggplot(gPCAdata_DEG_parent, aes(PC1, PC2, color=parent, shape=temperature)) + 
  ...
  geom_text(x=0, y=6, label="p[Interaction]=0.011", color="black")+
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     #panel.grid.major = element_blank(), #Set major gridlines 
                     #panel.grid.minor = element_blank(), #Set minor gridlines
...
```
There are 102 total genes and 101 unique genes, indicating that only one is shared between 30°C and 33°C.   
