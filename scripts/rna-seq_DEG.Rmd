...
temp30vs33_deg<-results(DEG, contrast=c("temperature","33","30"))
...
dim(temp30vs33_deg)
```
There are 213 genes differentially expressed between 27 and 30°C. There are 1160 genes differentially expressed between 27 and 33°C. There are 526 genes differentially expressed between 30 and 33°C. 1,899 genes total. In these contrasts, positive fold changes indicate higher expression in the first listed contrast.  
...
```
There are 1899 total genes and 1256 unique genes, indicating that some are shared in the contrasts. 
...
scaled_test_DEG_temp <-prcomp(test_DEG_temp[c(1:1256)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG_temp)

# scale data
vegan_DEG_temp <- scale(test_DEG_temp[c(1:1256)])
...
adonis2(formula = vegan_DEG_temp ~ parent * temperature, data = test_DEG_temp, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2     7864 0.11814  8.0768  0.001 ***
temperature         2    33890 0.50911 34.8073  0.001 ***
parent:temperature  4     2906 0.04366  1.4924  0.112    
Residual           45    21907 0.32910                   
Total              53    66568 1.00000                   
...
DEG_PCA_temp <- ggplot(gPCAdata_DEG_temp, aes(PC1, PC2, color=parent, shape=temperature)) + 
  geom_point(size=3) + 
  ggtitle(expression(bold("DEGs: temperature (1,256 genes)")))+
  xlab(paste0("PC1: ",percentVar_DEG_temp[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar_DEG_temp[2],"% variance")) +
  scale_color_manual(name="Parental Phenotype", values=c("Wildtype"="gray", "Bleached"="orange", "Nonbleached"="brown4"))+
  scale_shape_manual(name="Temperature", values=c(19,17,15))+
  geom_text(x=0, y=-10, label="p[Parent]=0.001", color="black")+
  geom_text(x=0, y=-11, label="p[Temperature]=0.001", color="black")+
  geom_text(x=0, y=-12, label="p[Interaction]=0.112", color="darkgray")+
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     #panel.grid.major = element_blank(), #Set major gridlines 
                     #panel.grid.minor = element_blank(), #Set minor gridlines
...
From this we can see very clear separation by temperature with high variance explained by PC1. The difference in parent exists in these DEG's, explained by PC2 with lower variance explained. It is interesting to note that there is more separation between bleached and the other two groups with little separation between nonbleached and wildtype. Wildtype and nonbleached both have mixed symbiont communities, while bleached are dominated by Cladocopium symbionts.  
...
BvsWT_deg<-results(DEG, contrast=c("parent","Bleached","Wildtype"))
...
NBvsWT_deg<-results(DEG, contrast=c("parent","Nonbleached","Wildtype"))
...
NBvsB_deg<-results(DEG, contrast=c("parent","Nonbleached","Bleached"))
...
dim(NBvsB_deg)
```
There are 599 genes differentially expressed between bleached and wildtype. There are 448 genes differentially expressed between nonbleached and wildtype. There are 671 genes differentially expressed between nonbleached and bleached. In these contrasts, positive fold changes indicate higher expression in the first listed contrast.  
...
```
There are 1718 total genes and 963 unique genes, indicating that some are shared in the contrasts. 
...
scaled_test_DEG_parent <-prcomp(test_DEG_parent[c(1:963)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG_parent)

# scale data
vegan_DEG_parent <- scale(test_DEG_parent[c(1:963)])
...
adonis2(formula = vegan_DEG_parent ~ parent * temperature, data = test_DEG_parent, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2    20111 0.39403 22.0201  0.001 ***
temperature         2     7225 0.14155  7.9106  0.001 ***
parent:temperature  4     3155 0.06181  1.7272  0.015 *  
Residual           45    20549 0.40261                   
Total              53    51039 1.00000                  
...
DEG_PCA_parent <- ggplot(gPCAdata_DEG_parent, aes(PC1, PC2, color=parent, shape=temperature)) + 
  geom_point(size=3) + 
  ggtitle(expression(bold("DEGs: parent (963 genes)")))+
  ...
  geom_text(x=0, y=10, label="p[Temperature]=0.001", color="black")+
  geom_text(x=0, y=8, label="p[Interaction]=0.015", color="black")+
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     #panel.grid.major = element_blank(), #Set major gridlines 
                     #panel.grid.minor = element_blank(), #Set minor gridlines
...
Next, view genes that are uniquely differentially expressed between parental types at 30°C and 33°C. In other words, view genes that are different between NB and B groups at 30°C and 33° that are not in the list of genes that are differentially expressed between parent groups regardless of temperature (#2 above). Conduct this for 30°C and then 33°C and then join the lists.   

To do this, I will use contrast DEGs between `temperature30.parentNonbleached`  and `temperature30.parentBleached` or `temperature33.parentNonbleached`  and `temperature33.parentBleached` in the DESeq2 results. This will examine the differential effect of parental treatment at 30°C and 33°C. Each of the NB and B parental groups have been compared to the wildtype group as a reference, so, this will contrast the comparisons of wildtype-NB and wildtype-B at the 30°C/33°C treatment. This will provide us with DEG's between parent groups at temperature. We will then keep only the genes that do NOT already appear in the DEG list between parental groups that we looked at in #2 above. This will produce a list of genes that are uniquely differentially expressed between B and NB at 30°C and 33°C.  
...
NBvsB_30_deg<-results(DEG, contrast=list("temperature30.parentNonbleached", "temperature30.parentBleached"))
NBvsB_30_deg <- as.data.frame(subset(NBvsB_30_deg, padj<0.01))
NBvsB_30_deg <- as.data.frame(subset(NBvsB_30_deg, abs(log2FoldChange)>1.5))
...
NBvsB_33_deg<-results(DEG, contrast=list("temperature33.parentNonbleached", "temperature33.parentBleached"))
NBvsB_33_deg <- as.data.frame(subset(NBvsB_33_deg, padj<0.01))
NBvsB_33_deg <- as.data.frame(subset(NBvsB_33_deg, abs(log2FoldChange)>1.5))
...
There are 367 DEGs between NB and B at 30°C. There are 475 at 33°C. In these contrasts, positive fold changes indicate higher expression in the first listed contrast - in this case, Nonbleached samples.  
...
```
There are 842 total genes and 653 unique genes, indicating that some are shared in between 30°C and 33°C.   

Remove any genes that are present in the DEG_parent list - in other words, remove genes that are different between parent regardless of temperature in order to keep the genes that are uniquely differentially expressed between NB and B under elevated temperature. 
...
```
There are 231 that are uniquely differentially expressed between NB and B at 30 or 33°C. 
...
dev.off()
```
There are 86 unique DEGs at 30°C and 171 at 33°C. Of these, 26 are shared, 145 are unique to 33°C and 60 are unique to 30°C.  
...
### Run a PERMANOVA and plot a PCA for interaction comparisons at 30°C.  REPEAT AT 33C 
...
scaled_test_DEG_interaction <-prcomp(test_DEG_interaction[c(1:231)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG_interaction)

# scale data
vegan_DEG_interaction <- scale(test_DEG_interaction[c(1:231)])
...
```
About 30% of variance is explained by first PC followed by 10% on the second PC and 5% on the third PC. 
...
Plot a PCA of DEG's for the interaction at 30°C 
...
DEG_PCA_interaction <- ggplot(gPCAdata_DEG_interaction, aes(PC1, PC2, color=parent, shape=temperature)) + 
  geom_point(size=3) + 
  ggtitle(expression(bold("DEGs: NB vs B at 30-33°C (231 genes)")))+
  xlab(paste0("PC1: ",percentVar_DEG_parent[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar_DEG_parent[2],"% variance")) +
  scale_color_manual(name="Parental Phenotype", values=c("Wildtype"="gray", "Bleached"="orange", "Nonbleached"="brown4"))+
  scale_shape_manual(name="Temperature", values=c(19,17,15))+
  geom_text(x=-3, y=-6, label="p[Parent]=0.001", color="black")+
  geom_text(x=-3, y=-7, label="p[Temperature]=0.001", color="black")+
  geom_text(x=-3, y=-8, label="p[Interaction]=0.001", color="black")+
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     #panel.grid.major = element_blank(), #Set major gridlines 
                     #panel.grid.minor = element_blank(), #Set minor gridlines
...
From this we can see very clear separation by parent at 30 and 33°C with little separation at 27°C. This is what we expected to see with DEGs that are different at elevated temperatures between parent types. The separation between parents is greater at 33°C than 30°C, which makes sense because we had a higher number of DEGs at 33°C than 30°C. 
