...
*pOverA*: Specifying the minimum count for a proportion of samples for each gene. Here, we are using a pOverA of 0.5, or 50% of samples. We are further setting the minimum count of genes to 10, such that 50% of the samples must have a gene count of >10 in order for the gene to remain in the data set. This prevents low count genes from being retained in the dataset and focuses on the most robust genes.   
...
Before filtering, we had approximately 48,332 genes. After filtering for pOverA, we have approximately 22,348 genes. This indicates that there were 22,348 genes present in 50% of samples at >10 counts per gene. 
...
dim(test)
scaled_test <-prcomp(test[c(1:22348)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test)

# scale data
vegan <- scale(test[c(1:22348)])
...
parent              2   172440 0.15130  6.1653  0.001 ***
temperature         2   299137 0.26246 10.6952  0.001 ***
parent:temperature  4    66832 0.05864  1.1947  0.133    
Residual           43   601339 0.52761                   
Total              51  1139748 1.00000      
...
allgenesfilt_PCA <- ggplot(gPCAdata, aes(PC1, PC2, color=parent, shape=temperature)) + 
  ...
  geom_text(x=0, y=-5.5, label="p[Phenotype]=0.001", color="black")+
  geom_text(x=0, y=-6.5, label="p[Temperature]=0.001", color="black")+
  geom_text(x=0, y=-7.5, label="p[Interaction]=0.140", color="darkgray")+
...
```{r}
allgenes_ellipse<-allgenesfilt_PCA + stat_ellipse(type="norm");allgenes_ellipse
...
```
There are 17,121 total genes and 9664 unique genes, indicating that some are shared in the contrasts. 
...
scaled_test_DEG_temp <-prcomp(test_DEG_temp[c(1:9664)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG_temp)

# scale data
vegan_DEG_temp <- scale(test_DEG_temp[c(1:9664)])
...
adonis2(formula = vegan_DEG_temp ~ parent * temperature, data = test_DEG_temp, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2    61027 0.12382  7.5161  0.001 ***
temperature         2   234596 0.47599 28.8928  0.001 ***
parent:temperature  4    22670 0.04600  1.3960  0.150    
Residual           43   174570 0.35420                   
Total              51   492864 1.00000        
...
DEG_PCA_temp <- ggplot(gPCAdata_DEG_temp, aes(PC1, PC2, color=parent, shape=temperature)) + 
  ...
  scale_shape_manual(name="Temperature", values=c(19,17,15))+
  geom_text(x=0, y=12.5, label="p[Phenotype]=0.001", color="black")+
  geom_text(x=0, y=11, label="p[Temperature]=0.001", color="black")+
  geom_text(x=0, y=9.5, label="p[Interaction]=0.150", color="darkgray")+
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     #panel.grid.major = element_blank(), #Set major gridlines 
                     #panel.grid.minor = element_blank(), #Set minor gridlines
...
```
There are 7011 total DEGs and 4675 unique genes, indicating that some are shared in the contrasts. 
...
scaled_test_DEG_parent <-prcomp(test_DEG_parent[c(1:4675)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG_parent)

# scale data
vegan_DEG_parent <- scale(test_DEG_parent[c(1:4675)])
...
adonis2(formula = vegan_DEG_parent ~ parent * temperature, data = test_DEG_parent, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2    81648 0.34245 21.5160  0.001 ***
temperature         2    62870 0.26369 16.5678  0.001 ***
parent:temperature  4    12320 0.05167  1.6232  0.033 *  
Residual           43    81587 0.34219                   
Total              51   238425 1.00000           
...
DEG_PCA_parent <- ggplot(gPCAdata_DEG_parent, aes(PC1, PC2, color=parent, shape=temperature)) + 
  ...
  scale_color_manual(name="Parental Phenotype", values=c("Wildtype"="gray", "Bleached"="orange", "Nonbleached"="brown4"), labels=parent_names)+
  scale_shape_manual(name="Temperature", values=c(19,17,15))+
  geom_text(x=-2, y=10, label="p[Phenotype]=0.001", color="black")+
  geom_text(x=-2, y=8, label="p[Temperature]=0.001", color="black")+
  geom_text(x=-2, y=6, label="p[Interaction]=0.033", color="black")+
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     #panel.grid.major = element_blank(), #Set major gridlines 
                     #panel.grid.minor = element_blank(), #Set minor gridlines
...
```
There are 6239 total genes and 4626 unique DEGs.
...
```
There are 4624 genes that are uniquely differentially expressed between NB and B at elevated temperature (30 or 33°C). 
...
scaled_test_DEG_interaction <-prcomp(test_DEG_interaction[c(1:4626)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG_interaction)

# scale data
vegan_DEG_interaction <- scale(test_DEG_interaction[c(1:4626)])
...
adonis2(formula = vegan_DEG_interaction ~ parent * temperature, data = test_DEG_interaction, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2    74769 0.31692 18.5741  0.001 ***
temperature         2    60079 0.25465 14.9247  0.001 ***
parent:temperature  4    14531 0.06159  1.8049  0.007 ** 
Residual           43    86547 0.36684                   
Total              51   235926 1.00000                      
...
DEG_PCA_interaction <- ggplot(gPCAdata_DEG_interaction, aes(PC1, PC2, color=parent, shape=temperature)) + 
  ...
  scale_shape_manual(name="Temperature", values=c(19,17,15))+
  geom_text(x=-2, y=-3.75, label="p[Parent]=0.001", color="black")+
  geom_text(x=-2, y=-5.25, label="p[Temperature]=0.001", color="black")+
  geom_text(x=-2, y=-6.75, label="p[Interaction]=0.007", color="black")+
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     #panel.grid.major = element_blank(), #Set major gridlines 
                     #panel.grid.minor = element_blank(), #Set minor gridlines
