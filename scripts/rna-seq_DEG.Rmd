...
Before filtering, we had approximately 44,690 genes. After filtering for pOverA, we have approximately 24,464 genes. This indicates that there were 24,464 genes present in 10% of samples at <10 counts per gene. 6/54 = 0.11, so I selected 10% filter because I want to keep genes that are uniquely expressed in one treatment group (n=6 out of 54).  
...
dim(test)
scaled_test <-prcomp(test[c(1:24464)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test)

# scale data
vegan <- scale(test[c(1:24464)])
...
parent              2   156619 0.12079 4.3807  0.001 ***
temperature         2   255656 0.19718 7.1508  0.001 ***
parent:temperature  4    79895 0.06162 1.1174  0.189    
Residual           45   804421 0.62041                  
Total              53  1296592 1.00000   
...
allgenesfilt_PCA <- ggplot(gPCAdata, aes(PC1, PC2, color=parent, shape=temperature)) + 
  geom_point(size=3) + 
  ggtitle(expression(bold("All genes (24,464 genes)")))+
  ...
  geom_text(x=0, y=7, label="p[Interaction]=0.189", color="darkgray")+
...
There are 991 genes differentially expressed between 27 and 30°C. There are 6517 genes differentially expressed between 27 and 33°C. There are 3637 genes differentially expressed between 30 and 33°C. In these contrasts, positive fold changes indicate higher expression in the first listed contrast.  
...
```
There are 11,145 total genes and 6,849 unique genes, indicating that some are shared in the contrasts. 
...
Most DEGs are those that are differentially expressed at 33°C (27 vs 33°C and 30 vs 33°C). There are far fewer that are differential between 27 and 30°C. 

### Plot a volcano plot for these comparisons.   

27 vs 30°C
```{r}
res <- results(DEG_T, contrast=c("temperature","30","27"))
...
27 vs 33°C
```{r}
res <- results(DEG_T, contrast=c("temperature","33","27"))
...
30 vs 33°C
```{r}
res <- results(DEG_T, contrast=c("temperature","33","30"))
...
scaled_test_DEG_temp <-prcomp(test_DEG_temp[c(1:6849)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG_temp)

# scale data
vegan_DEG_temp <- scale(test_DEG_temp[c(1:6849)])
...
adonis2(formula = vegan_DEG_temp ~ parent * temperature, data = test_DEG_temp, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2    37221 0.10254  5.8984  0.001 ***
temperature         2   166961 0.45995 26.4582  0.001 ***
parent:temperature  4    16832 0.04637  1.3336  0.136    
Residual           45   141983 0.39114                   
Total              53   362997 1.00000                   
...
DEG_PCA_temp <- ggplot(gPCAdata_DEG_temp, aes(PC1, PC2, color=parent, shape=temperature)) + 
  ...
  scale_color_manual(name="Parental Phenotype", values=c("Wildtype"="gray", "Bleached"="orange", "Nonbleached"="brown4"))+
  scale_shape_manual(name="Temperature", values=c(19,17,15))+
  geom_text(x=0, y=-5, label="p[Parent]=0.001", color="black")+
  geom_text(x=0, y=-6.5, label="p[Temperature]=0.001", color="black")+
  geom_text(x=0, y=-8, label="p[Interaction]=0.136", color="darkgray")+
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     #panel.grid.major = element_blank(), #Set major gridlines 
                     #panel.grid.minor = element_blank(), #Set minor gridlines
...
dim(NBvsB_deg)
```
There are 1109 genes differentially expressed between bleached and wildtype. There are 1174 genes differentially expressed between nonbleached and wildtype. There are 1082 genes differentially expressed between nonbleached and bleached. In these contrasts, positive fold changes indicate higher expression in the first listed contrast.  
...
```
There are 3365 total DEGs and 2369 unique genes, indicating that some are shared in the contrasts. 
...
There are a large number of DEGs (>400) for each comparison. There are 324 DEGs between WT and both B and NB, which I expect may be due to population/genetic effects. There are 320 DEGs between B and all other groups and 256 DEGs between NB and all other groups. 
...
It may be worth examining sample R93 as an outlier. 
...
scaled_test_DEG_parent <-prcomp(test_DEG_parent[c(1:2369)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG_parent)

# scale data
vegan_DEG_parent <- scale(test_DEG_parent[c(1:2369)])
...
adonis2(formula = vegan_DEG_parent ~ parent * temperature, data = test_DEG_parent, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2    45111 0.35928 23.0004  0.001 ***
temperature         2    29910 0.23822 15.2503  0.001 ***
parent:temperature  4     6407 0.05103  1.6333  0.011 *  
Residual           45    44129 0.35147                   
Total              53   125557 1.00000            
...
DEG_PCA_parent <- ggplot(gPCAdata_DEG_parent, aes(PC1, PC2, color=parent, shape=temperature)) + 
  ...
  geom_text(x=0, y=6, label="p[Interaction]=0.013", color="black")+
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     #panel.grid.major = element_blank(), #Set major gridlines 
                     #panel.grid.minor = element_blank(), #Set minor gridlines
...
I am changing the p-value to 0.05 for this test. 
...
There are 7 DEGs between NB and B at 30°C. There are 95 at 33°C. In these contrasts, positive fold changes indicate higher expression in the first listed contrast - in this case, Nonbleached samples.  
...
```
There are 102 total genes and 101 unique genes, indicating that only one is shared between B and NB at the 30°C and 33°C temperature contrasts.   
...
```
There are 101 genes that are uniquely differentially expressed between NB and B at elevated temperature (30 or 33°C). 
...
dev.off()
```
There are 94 DEGs at 33°C and 6 at 30°C with 1 shared between the contrasts.   
...
Plot heatmap.  
```{r}
pdf("figures/rna_seq/DEG_heatmap_interaction.pdf", width=6, height=8)
...
scaled_test_DEG_interaction <-prcomp(test_DEG_interaction[c(1:101)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG_interaction)

# scale data
vegan_DEG_interaction <- scale(test_DEG_interaction[c(1:101)])
...
adonis2(formula = vegan_DEG_interaction ~ parent * temperature, data = test_DEG_interaction, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2   1191.5 0.22259 16.4227  0.001 ***
temperature         2   1917.5 0.35821 26.4290  0.001 ***
parent:temperature  4    611.5 0.11424  4.2143  0.001 ***
Residual           45   1632.4 0.30496                   
Total              53   5353.0 1.00000                   
...
DEG_PCA_interaction <- ggplot(gPCAdata_DEG_interaction, aes(PC1, PC2, color=parent, shape=temperature)) + 
  geom_point(size=3) + 
  ggtitle(expression(bold("DEGs: NB vs B at 30-33°C (101 genes)")))+
  xlab(paste0("PC1: ",percentVar_DEG_parent[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar_DEG_parent[2],"% variance")) +
  scale_color_manual(name="Parental Phenotype", values=c("Wildtype"="gray", "Bleached"="orange", "Nonbleached"="brown4"))+
  scale_shape_manual(name="Temperature", values=c(19,17,15))+
  geom_text(x=0, y=-1.5, label="p[Parent]=0.001", color="black")+
  geom_text(x=0, y=-2, label="p[Temperature]=0.001", color="black")+
  geom_text(x=0, y=-2.5, label="p[Interaction]=0.001", color="black")+
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     #panel.grid.major = element_blank(), #Set major gridlines 
                     #panel.grid.minor = element_blank(), #Set minor gridlines
...
From this we can see very clear separation by parent at 33°C with less separation and higher variability in the other groups especially at 27°C. This is what we expected to see with DEGs that are different at elevated temperatures between parent types. 
