...
LEFT OFF WITH INCREASING FOLD CHANGE AND ADDING UP AND DOWN REGULATED DFS 
...
# Conduct PERMANOVA and PCA by parent and temperature for all genes
...
DEG.results.all <- as.data.frame(subset(DEG.results.all, abs(log2FoldChange)>1.5))
dim(DEG.results.all)
...
```
There are 387 genes differentially expressed at p<0.01 and log2FoldChange>1.5 between temperature x parent groups. Because we did not have interactive effects in the PERMANOVA, it makes sense that this is a lower number than within temperature and within parent comparisons that are done below. 
...
scaled_test_DEG <-prcomp(test_DEG[c(1:387)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG)

# scale data
vegan_DEG <- scale(test_DEG[c(1:387)])
...
adonis2(formula = vegan_DEG ~ parent * temperature, data = test_DEG, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2   4939.3 0.24081 12.2885  0.001 ***
temperature         2   4910.6 0.23941 12.2172  0.001 ***
parent:temperature  4   1617.3 0.07885  2.0119  0.001 ***
Residual           45   9043.7 0.44092                   
Total              53  20511.0 1.00000    
...
DEG_PCA <- ggplot(gPCAdata_DEG, aes(PC1, PC2, color=parent, shape=temperature)) + 
  geom_point(size=3) + 
  ggtitle(expression(bold("DEGs: all groups (391 genes)")))+
...
# View heatmap by parent and treatment  
...
Compare NB in numerator and B in denominator (positive FC = upregulated in NB group). 
```{r}
resultsNames(DEG)
...
parent_degNB_B <- as.data.frame(subset(parent_degNB_B, abs(log2FoldChange)>1.5))

dim(parent_degNB_B)
```
There are 463 genes differentially expressed between nonbleached and bleached parents at p<0.01 and l2FC>1.5. 
...
parent_parent_degNB_WT <- as.data.frame(subset(parent_parent_degNB_WT, abs(log2FoldChange)>1.5))

dim(parent_parent_degNB_WT)
```
There are 259 genes differentially expressed between nonbleached and wildype parents at p<0.01 and l2FC>1.5. 
...
parent_parent_degB_WT <- as.data.frame(subset(parent_parent_degB_WT, abs(log2FoldChange)>1.5))

dim(parent_parent_degB_WT)
```
There are 396 genes differentially expressed between wildtype and bleached parents at p<0.01 and l2FC>1.5. 
## Temperature comparisons 

Next, look at 27 vs 30 degrees and 27 vs 33 degrees. 

27 vs 30 degrees. Compare 30 in numerator and 27 in denominator (positive FC = upregulated in 20 group). 
```{r}
...
dim(temp30_deg)
```
There are 128 DEGs between 27 and 30 degrees. 
...
dim(temp33_deg)
```
There are 574 DEGs between 27 and 33 degrees. 
...
parent_deg_int1 <- as.data.frame(subset(parent_deg_int1, abs(log2FoldChange)>1.5))

dim(parent_deg_int1)
```
There are 302 genes differentially expressed genes in the interaction between temperature and parent. This shows DEG's contrasted between NB and B at 33°C. 
...
scaled_test_int <-prcomp(test_int[c(1:302)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_int)

# scale data
vegan_int <- scale(test_int[c(1:302)])
...
adonis2(formula = vegan_int ~ parent * temperature, data = test_int, method = "eu")
                   Df SumOfSqs      R2       F Pr(>F)    
parent              2   4384.9 0.27395 12.8111  0.001 ***
temperature         2   2535.3 0.15840  7.4074  0.001 ***
parent:temperature  4   1384.7 0.08651  2.0228  0.002 ** 
Residual           45   7701.1 0.48114                   
Total              53  16006.0 1.00000  
...
int_PCA <- ggplot(gPCAdata_int, aes(PC1, PC2, color=parent, shape=temperature)) + 
  geom_point(size=3) + 
  ggtitle(expression(bold("DEGs: NB vs B at 33°C (302 genes)")))+
...
parent_deg_int3 <- as.data.frame(subset(parent_deg_int3, abs(log2FoldChange)>1.5))

dim(parent_deg_int3)
```
There are 302 of these genes. 

List of genes in NB vs B overall 
```{r}
resultsNames(DEG)
parent_deg_int4<-results(DEG, contrast=list("parent_Nonbleached_vs_Wildtype", "parent_Bleached_vs_Wildtype"))
parent_deg_int4 <- as.data.frame(subset(parent_deg_int4, padj<0.01))
parent_deg_int4 <- as.data.frame(subset(parent_deg_int4, abs(log2FoldChange)>1.5))

dim(parent_deg_int4)
```
There are 463 of these genes.
...
There are 150 genes unique to 33°C NB vs B comparisons compared to overall NB vs B comparisons. These genes represent uniquely thermally responsive genes by parental phenotype. 
...
scaled_test_unique <-prcomp(test_unique[c(1:150)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_unique)

# scale data
vegan_unique <- scale(test_unique[c(1:150)])
...
adonis2(formula = vegan_unique ~ parent * temperature, data = test_unique, method = "eu")
                   Df SumOfSqs      R2      F Pr(>F)    
parent              2   1733.7 0.21807 9.5673  0.001 ***
temperature         2   1294.2 0.16279 7.1419  0.001 ***
parent:temperature  4    845.0 0.10629 2.3317  0.001 ***
Residual           45   4077.1 0.51285                  
Total              53   7950.0 1.00000   
...
unique_PCA <- ggplot(gPCAdata_unique, aes(PC1, PC2, color=parent, shape=temperature)) + 
  geom_point(size=3) + 
  ggtitle(expression(bold("DEGs: Unique DEGs NB vs B at 33°C (150 genes)")))+
...
SPLIT WITH UP VS DOWN REGULATED  


1. DEGs by NB vs B parent (DEGvstNB_B) = 1114 genes 

Upregulated in NB compared to B (619 genes). 
```{r}
up_list<-parent_degNB_B%>%
  filter(log2FoldChange>0)

up_list<-rownames(up_list)
length(up_list)

dim(DEGvstNB_B)

parent_DEGs<-t(assay(DEGvstNB_B))
parent_DEGs<-as.data.frame(parent_DEGs)
parent_DEGs$sample<-rownames(parent_DEGs)
parent_DEGs<-parent_DEGs%>%
  select(sample, everything())

dim(parent_DEGs)

parent_DEGs_up <- parent_DEGs[, intersect(up_list, colnames(parent_DEGs))]

dim(parent_DEGs_up)

write_csv(file="output/rna_seq/parent_DEGs_up.csv", parent_DEGs_up)
```

Downregulated in NB compared to B. 
```{r}
down_list<-parent_degNB_B%>%
  filter(log2FoldChange<0)

down_list<-rownames(down_list)
length(down_list)
...
parent_DEGs_down <- parent_DEGs[, intersect(down_list, colnames(parent_DEGs))]

dim(parent_DEGs_down)

write_csv(file="output/rna_seq/parent_DEGs_down.csv", parent_DEGs_down)
```
...
2. DEGs at 27 vs 33 temperature (DEGvst27_33) = 2533 genes

```{r}
dim(DEGvst27_33)
temp_DEGs<-t(assay(DEGvst27_33))
temp_DEGs<-as.data.frame(temp_DEGs)
dim(temp_DEGs)
temp_DEGs$sample<-rownames(temp_DEGs)
temp_DEGs<-temp_DEGs%>%
  select(sample, everything())

write_csv(file="output/rna_seq/temp_DEGs.csv", temp_DEGs)
```

3. DEGs uniquely differential between B and NB at 33°C (DEGvst_unique) = 474 genes

```{r}
#subset for up vs down regualted 
str(unique_genes)

up_unique<-unique_genes%>%
  filter(log2FoldChange>1) #positive values = up regulated in the group listed first in the model; negative values = down regulated in the group listed second in the model



dim(DEGvst_unique)
unique_DEGs<-t(assay(DEGvst_unique))
unique_DEGs<-as.data.frame(unique_DEGs)
dim(unique_DEGs)
unique_DEGs$sample<-rownames(unique_DEGs)
unique_DEGs<-unique_DEGs%>%
  select(sample, everything())

write_csv(file="output/rna_seq/unique_NBvsB_33C_DEGs.csv", unique_DEGs)
```

4. List of all genes in the dataset (gvst) = 24005 genes.  
