...
metric_plot2<-pr_data_calc%>%
    ...
    #filter(!c(Metric=="ratio_low" & Value>8))%>%
...
summary(model)
```
No effect of temp or group on P:R ratio at high light. 
...
summary(model)
```
No effect on P:R ratio at medium light. 
...
pairs(emm)
```
Light enhanced respiration is reduced with high temperature very strongly. LER is lowest at 36C. 
...
TukeyHSD(model, "Group")
```
P net is lower in F1 NB at low light, but this is barely significant (p=0.05). 
...
```
P:R ratios are significantly different by light. There are higher P:R ratios at high light. 
