...
```
LER is dfiferent by group. F2 NB has lower respiration than F1 NB. F2 NB also has lower Rd than F2 WT.  
...
TukeyHSD(model, "Group")
```
Dark respiration decreases with temperature. 
...
TukeyHSD(model, "Group")
```
At medium light, P gross decreases with temperature and is different by group. F2 WT has higher P than F2 NB. There is lower P in F2 NB than F1 NB. 
...
TukeyHSD(model, "Group")
```
Gross photosynthesis at low light is affected by group. 
...
TukeyHSD(model, "Group")
```
P net is is affected by group at low light. 
...
pairs(emm)
```
There are interactive effects between light and temp and between temp and group. 
