...
```{r}
dark26_grid<-plot_grid(dark_26c_plot2, dark_26c_QY_plot2, dark_26c_NPQ_plot2, 
                    ncol=3, nrow=1, align="vh", axis = "lr")

ggsave(filename="figures/phenoplate/1_dark_26c_metrics.jpeg", dark26_grid, width=15, height=5)
```
...
### Plot all for this phase together 
```{r}
light26_grid<-plot_grid(light_26c_plot2, light_26c_QY_plot2, light_26c_NPQ_plot2, 
                    ncol=3, nrow=1, align="vh", axis = "lr")

ggsave(filename="figures/phenoplate/2_light_26c_metrics.jpeg", light26_grid, width=15, height=5)
```

## 3. Dark [Temperature Gradient]: Light enhanced respiration, NPQ, and QY ~ Temperature and phenotype
...
Summarize QY values in last 5 minutes. Calculate as relative change in Fm per min. 
```{r}
QY_gradient<-df%>%
  filter(Phase=="Dark - Temp Gradient")%>%
  filter(Time>=35)%>%
  group_by(Plate, well, code, Symbiont, Gradient_Temperature)%>%
  summarise(QYgradient=mean(QY, na.rm=TRUE)/15)
```
...
QY_gradient_plot2<-QY_gradient%>%
  ...
  geom_text(x=30, y=0.0495, label="p[temp]<0.001", size=6, color="black")+
  geom_text(x=30, y=0.048, label="p[parent]<0.001", size=6, color="black")+
  geom_text(x=30, y=0.0465, label="p[interaction]=0.479", size=6, color="darkgray")+
  scale_colour_manual(values=c("orange", "brown4", "gray"), name="Symbiont")+
  scale_fill_manual(values=c("orange", "brown4", "gray"), name="Symbiont")+
  ylim(0.03,0.05)+
  theme_classic()+
...
NPQ_gradient<-df%>%
  ...
  filter(Time>=35)%>%
  group_by(Plate, well, code, Symbiont, Gradient_Temperature)%>%
  summarise(NPQgradient=mean(NPQ_norm, na.rm=TRUE)/15)
```
...
NPQ_gradient_plot2<-NPQ_gradient%>%
  ...
  xlab("Temperature")+
  ylab(expression(bold(paste("Non-Photochemical Quenching"))))+
  ggtitle(expression(bold(paste("NPQ Temp. Gradient"))))+
  geom_text(x=30, y=0.051, label="p[temp]<0.001", size=6, color="black")+
  geom_text(x=30, y=0.047, label="p[parent]<0.001", size=6, color="black")+
  geom_text(x=30, y=0.043, label="p[interaction]<0.001", size=6, color="black")+
...
### Plot all for this phase together 
```{r}
gradient_grid<-plot_grid(LEDR_gradient_plot2, QY_gradient_plot2, NPQ_gradient_plot2, 
                    ncol=3, nrow=1, axis = "lr", rel_widths=c(1,1,1.25))
