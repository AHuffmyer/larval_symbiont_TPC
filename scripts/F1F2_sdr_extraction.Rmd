---
title: "Larval Photosynthesis Rates Extraction with LoLinR - Hawaii 2023 - F1F2 project"
author: "AS Huffmyer"
date: '2023'
output:
  html_document:
...
```{r, warning=FALSE, message=FALSE}
path.p<-"data/F1F2_sdr/runs" #location of files
```
...
```{r}
run_metadata<-read.csv("data/F1F2_sdr/F1F2_SDR_Run_Info.csv")
```
...
Sample.Info <- read_csv(file = "data/F1F2_sdr/F1F2_SDR_Sample_Info.csv")%>%
  select(!Notes)
...
date <- str_sub(file.names, 4, str_length(file.names)-30) #grab date from file name

# Load PI curve run metadata (i.e., light levels and interval times for each run)
Run.Info <- read_csv(file = "data/F1F2_sdr/F1F2_SDR_Run_Info.csv")
...
THE SCRIPT IS ONLY USING THE FIRST SET OF CUT OFF VALUES!!! NOT RUNNING DOWN THE LIST OF CUT OFF VALUES 
...
write.csv(Temp.P, paste0("output/F1F2_sdr/runs_temp.csv")) #save respiration rate data
```
...
write.csv(Photo.R, paste0("output/F1F2_sdr/F1F2_sdr_rates_raw.csv")) #save respiration rate data
...
```{r, warning=FALSE, message=FALSE}
Photo.Rates <- read.csv(file = "output/F1F2_sdr/F1F2_sdr_rates_raw.csv") #read file back in so slopes don't have to be generated every time
...
oxygen$Group<-Sample.Info$Group[match(oxygen$unique, Sample.Info$unique)]
...
oxygen<-oxygen%>%
  ...
  filter(!c(code=="2 A2" & Light==5))%>%
  filter(!c(code=="2 A5" & Light==2))%>%
  filter(!c(code=="2 A6" & Light==1))%>%
  filter(!c(code=="2 C4" & Light==2))%>%
  #filter(!code=="12 A3")%>%
  #filter(!code=="12 B5")%>%
  #filter(!code=="13 D6")%>%
  #filter(!code=="15 A4")%>%
...
oxygen$code<-paste0(oxygen$Plate, "-", oxygen$Light)
```
...
blank_data$code<-paste0(blank_data$Plate, "-", blank_data$Light)
...
Save as .csv file.  
```{r, warning=FALSE, message=FALSE}
write.csv(oxygen.bio, paste0("output/F1F2_sdr/F1F2_calculated_normalized_photo_rates.csv")) #save final file
```
