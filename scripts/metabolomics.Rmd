...
135 metabolites are kept after 50% of samples >0 filtering, many were removed. This makes sense, the data frame kept all metabolites in the library so we expect far fewer to be retained.   
...
```{r}
plot1<-filtered_data%>%s
  filter(Name %in% c("Propionic acid", "Butyric acid-TMS"))%>%
  
  
  ggplot(aes(x=paste(parent), y=area_normalized))+
  facet_wrap(~Name, scales="free")+
  geom_point()+
  theme_classic()+
