...
Sample L107 is a clear outlier. Remove this sample and plot again. 
...
Remove metabolites that have RSD < 100 in all treatments (RSD values are high, come back to this).  
```{r}
test <- test %>%
  group_by(Name) %>%
  filter(all(RSD < 100))
...
```{r}
scaled.pca<-prcomp(wide_data[c(4:39)], scale=TRUE, center=TRUE) 
...
parent              2    53.70 0.02868 0.7731  0.561  
temperature         2    54.56 0.02914 0.7855  0.552  
parent:temperature  4   235.77 0.12594 1.6973  0.080 .
Residual           44  1527.98 0.81623                
Total              52  1872.00 1.00000       
...
X<-X[4:39] #pull only data columns
...
ggsave(interaction_means, filename="figures/metabolites/interaction_VIP_means.jpeg", width=8, height=6)
```
