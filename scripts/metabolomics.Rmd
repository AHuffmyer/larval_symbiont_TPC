...
NEED TO SELECT HIGHEST SIGNAL FOR DUPLICATE METABOLITE ENTRIES 
NEED TO TROUBLESHOOT AND ADJUST METABOLITE DATA RSD FILTERING 
NEED TO APPLY SAM OR OTHER MODELS 
...
66 are kept with filtering for >0 value in 90% of samples. Keep these for now. 
...
Plot raw values for high RSD metabolites. 

```{r}
rsd_list<-test_heatmap%>%
  filter(RSD>100)%>%
  pull(Name)%>%
  unique()
```

There are 27 metabolites with RSD > 100. Plot these. 

```{r}
rsd_plot1<-filtered_data%>%
  filter(Name %in% rsd_list)%>%
  
  ggplot(aes(x=paste(temperature, parent), y=area_normalized))+
  facet_wrap(~Name, scales="free")+
  geom_point()+
  theme_classic()+
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1));rsd_plot1

rsd_plot2<-filtered_data%>%
  filter(Name %in% c("Trehalose-8TMS"))%>%
  
  ggplot(aes(x=paste(temperature, parent), y=area_normalized, label = tube))+
  facet_wrap(~Name, scales="free")+
  geom_point()+
  geom_text(hjust=1.5)+
  theme_classic()+
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1));rsd_plot2

rsd_plot3<-filtered_data%>%
  filter(Name %in% c("Sucrose-8TMS"))%>%
  ...
  geom_text(hjust=1.5)+
  theme_classic()+
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1));rsd_plot3

rsd_plot3<-filtered_data%>%
  filter(Name %in% c("Monostearin-2TMS"))%>%
...
The high RSD in the wildtype high temp group may be driven by one sample - L59. 

Remove this sample and check again. 
...
Plot concentrations of glucose - (2) does have higher RSD than (1). See which has higher concentration
```{r}
rsd_plot1<-filtered_data%>%
  filter(Name %in% c("Glucose-meto-5TMS(1)", "Glucose-meto-5TMS(2)"))%>%
...
```{r}
rsd_plot1<-filtered_data%>%
  filter(Name %in% c("Glycine-2TMS", "Glycine-3TMS"))%>%
...
Remove glucose (2) and glycine 3TMS. 
```{r}
filtered_data<-filtered_data%>%
  filter(!Name %in% c("Glycine-3TMS", "Glucose-meto-5TMS(1)"))
```
...
donis2(formula = vegan ~ parent * temperature, data = wide_data, method = "eu")
                   Df SumOfSqs      R2      F Pr(>F)
parent              2    70.54 0.03083 0.8258  0.514
temperature         2    65.22 0.02850 0.7635  0.553
parent:temperature  4   273.08 0.11935 1.5985  0.111
Residual           44  1879.16 0.82131              
Total              52  2288.00 1.00000           
...
```{r}
scaled.pca<-prcomp(vip_data_interaction %>% select(where(is.numeric)), scale=TRUE, center=TRUE)
...
```{r}
# scale data
vegan <- scale(vip_data_interaction %>% select(where(is.numeric)))
...
model<-aov(area_normalized ~ Name * temperature * parent, data=model_data)
