...
First calculate Fm for each well as average of values from the first 15 min dark period. Then calculate normalized NPQ as (Fm-value)/value. Note that below we use NPQ_raw values. 
...
c_o2_npq_plot<-df%>%
  filter(Symbiont=="C")%>%
  group_by(Symbiont, Gradient_Temperature, Time, Light)%>%
  summarise(mean=mean(NPQ_raw, na.rm=TRUE), se=sd(NPQ_raw, na.rm=TRUE)/sqrt(length(NPQ_raw)))%>%
...
light_26c_NPQ_plot1<-df%>%
  ...
  ggplot(aes(x=Time, y=NPQ_raw, colour=Symbiont, fill=Symbiont))+
  geom_point()+
  xlab("Time (min)")+
  ylab("NPQ")+
  ggtitle("Light 26°C")+
...
NPQ_light26<-df%>%
  ...
  filter(Time>=25)%>%
  group_by(Plate, well, code, Symbiont)%>%
  summarise(NPQlight26=mean(NPQ_raw, na.rm=TRUE))
```
...
light_26c_NPQ_plot2<-NPQ_light26%>%
  ...
  geom_text(x=2, y=1000, label="p=0.142", size=6, color="black")+
  #geom_text(x=1, y=6800, label="a", size=6, color="black")+
  #geom_text(x=2, y=6400, label="a", size=6, color="black")+
  #geom_text(x=3, y=5800, label="b", size=6, color="black")+
  scale_colour_manual(values=c("orange", "brown4", "gray"), name="Parent (Symbiont)", labels=parent_names)+
  ggtitle("B")+
  scale_x_discrete(labels=parent_names)+
  ylim(0, 7500)+
...
low_light_26c_NPQ_plot2<-df%>%
  ...
  ggplot(aes(x=Gradient_Temperature, y=NPQ_raw, colour=Symbiont))+
  geom_boxplot(position=position_dodge(0.5))+
  geom_point(size=3, alpha=0.5, position = position_dodge(0.5))+
  xlab("Temperature (°C)")+
  ylab("Non-Photochemical Quenching")+
  geom_text(x=6, y=2000, label="p[temperature x phenotype]=0.001", size=4, color="black")+
  ggtitle("D")+
  scale_colour_manual(values=c("orange", "brown4", "gray"), name="Symbiont")+
  scale_fill_manual(values=c("orange", "brown4", "gray"), name="Symbiont")+
  theme_classic()+
...
model<-df%>%
  ...
  aov(NPQ_raw~Symbiont*Gradient_Temperature, data=.)
...
Wildtype different than other phenotypes. 
