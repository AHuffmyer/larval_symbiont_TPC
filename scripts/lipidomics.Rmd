...
```{r}
levels(as.factor(lipids$ontology))

ontology<-lipids%>%ungroup()%>%select(lipid, formula, ontology)%>%unique()
```
...
```{r}
vip_data_parent <- vip_data_parent[, colSums(vip_data_parent != 0) > 0]
```
This retained 342 lipid VIPs.

View scree plot. 

```{r}
scaled.pca<-prcomp(vip_data_parent[c(4:345)], scale=TRUE, center=TRUE) 
...
```{r}
# scale data
vegan <- scale(vip_data_parent[c(4:345)])
...
adonis2(formula = vegan ~ parent * temperature, data = vip_data_parent, method = "eu")
                   Df SumOfSqs      R2      F Pr(>F)    
parent              2   3172.3 0.17838 5.8133  0.001 ***
temperature         2   1356.7 0.07629 2.4861  0.016 *  
parent:temperature  4   1249.5 0.07026 1.1449  0.285    
Residual           44  12005.5 0.67507                  
Total              52  17784.0 1.00000     
...
```{r}
vip_data_temperature <- vip_data_temperature[, colSums(vip_data_temperature != 0) > 0]
```
This retained 405 lipid VIPs.

View scree plot. 

```{r}
scaled.pca<-prcomp(vip_data_temperature[c(4:408)], scale=TRUE, center=TRUE) 
...
```{r}
# scale data
vegan <- scale(vip_data_temperature[c(4:408)])
...
adonis2(formula = vegan ~ parent * temperature, data = vip_data_temperature, method = "eu")
                   Df SumOfSqs      R2      F Pr(>F)   
parent              2   1447.8 0.06875 2.0692  0.043 * 
temperature         2   2215.3 0.10519 3.1661  0.007 **
parent:temperature  4   2003.3 0.09512 1.4315  0.120   
Residual           44  15393.6 0.73094                 
Total              52  21060.0 1.00000     
...
```{r}
vip_data_interaction <- vip_data_interaction[, colSums(vip_data_interaction != 0) > 0]
```
This retained 463 lipid VIPs.

View scree plot. 

```{r}
scaled.pca<-prcomp(vip_data_interaction[c(4:466)], scale=TRUE, center=TRUE) 
...
```{r}
# scale data
vegan <- scale(vip_data_interaction[c(4:466)])
...
adonis2(formula = vegan ~ parent * temperature, data = vip_data_interaction, method = "eu")
                   Df SumOfSqs      R2      F Pr(>F)    
parent              2   3452.4 0.14340 4.6297  0.001 ***
temperature         2   2135.0 0.08868 2.8631  0.003 ** 
parent:temperature  4   2082.9 0.08651 1.3966  0.091 .  
Residual           44  16405.7 0.68141                  
Total              52  24076.0 1.00000   
...
Significant enrichment of monounsaturated fatty acids, fatty acids, fatty acids and conjugates, diacylglycerols, fatty acid with less than 2 double bonds, and lipid-mediated signalling.  
...
#32 fatty acids in this data set 
...
dev.off()
pdf("figures/lipids/diacylglycerols_VIP.pdf", width=6, height=14)
draw(heatmap)
dev.off()
```
...
#93 fatty acids in this data set 
...
dev.off()
pdf("figures/lipids/lipid_signaling_VIP.pdf", width=6, height=14)
draw(heatmap)
dev.off()
```
