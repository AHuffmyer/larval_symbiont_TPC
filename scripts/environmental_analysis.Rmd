...
```{r}
daily<-read_csv("data/environmental/example_daily_measurements.csv")
...
```{r}
pHcalib<-read_csv("data/environmental/example_Tris_Calibration.csv")
...
```{r}
daily_calc<-pHSlope%>%
  select(date, time, tank, symbiont, temp.C, pH, sal.psu, flow.mL.5s, par)
```
...
daily_tanks<-daily_calc.long %>%
  ggplot(aes(x=date, y=value, colour=symbiont))+
  geom_point(size=2)+
...
daily_treatment<-daily_calc.long %>%
  ggplot(aes(x=date, y=value, colour=symbiont, group=interaction(symbiont, date)))+
  geom_point()+
  geom_boxplot()+
  #geom_vline(xintercept = as.numeric(ymd("2022-10-19")), linetype="dashed", 
                #color = "black", size=0.5)+
...
```{r}
daily_calc.short%>%
  aov(flow.L.min~symbiont, data=.)%>%
  summary()
 
daily_calc.short%>%
  aov(par~symbiont, data=.)%>%
  summary()

daily_calc.short%>%
  aov(pH~symbiont, data=.)%>%
  summary()

daily_calc.short%>%
  aov(sal.psu~symbiont, data=.)%>%
  summary()

daily_calc.short%>%
  aov(temp.C~symbiont, data=.)%>%
  summary()
```
...
# Logger analysis (NEED TO EDIT WITH TIDBIT FILE STRUCTURE)   
...
## Apply tempreature calibrations (wont need calibrations)  
