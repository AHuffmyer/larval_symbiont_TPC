...
Run full analysis to obtain 1000 successful validation runs (dealing with SVD error)
```{r}
# Target number of successful validation runs
target_success <- 1000

# How many validation runs to attempt per chunk
chunk_runs <- 250

# Safety cap so you don’t loop forever if something is fundamentally wrong
max_attempts <- 20  # 20 * 250 = 5000 attempted runs maximum
...
set.seed(123)  # for reproducibility of the resampling sequence

while (success_total < target_success && attempt < max_attempts) {
  attempt <- attempt + 1
  message("Attempt ", attempt, " (trying ", chunk_runs, " validation runs)...")

  fit_i <- tryCatch(
    ALASCA(
      long_data,
      value ~ temperature * parent + (1|sample),
      n_validation_runs = chunk_runs,
      validate = TRUE,
      reduce_dimensions = TRUE
    ),
    error = function(e) {
  ...
  if (!is.null(fit_i)) {
    ...
    success_total <- success_total + chunk_runs
    message("  -> success_total = ", success_total)
  }
}

if (success_total < target_success) {
  stop("Could not reach ", target_success,
       " successful validation runs. Reached ", success_total,
       " after ", attempt, " attempts.")
}

# Use the last successful fit object for downstream plotting/loadings
# (Validation was performed; this object is consistent with your existing workflow.)
res_all <- fits[[length(fits)]]
