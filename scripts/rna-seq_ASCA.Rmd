...
Conduct data filtering using pOverA approach. Filter to keep genes at counts >10 in at least 9% of samples. 
...
Remove the two outlier samples detected in unsupervised analysis. 
...
#Different between wildtype and other parents at 30-33C 
...
#Different between wildtype/bleached and nonbleached at 30-33
...
importance<-importance%>%select(gene, abs_loading, var_contrib, PC)%>%rename("variable"=gene)
head(importance)
...
pc_plot7<-pc_conc%>%
  ...
  #group_by(temperature, parent, variable)%>%
  ...
  geom_boxplot(outliers=FALSE)+
  geom_point(position=position_dodge(0.8))+
  #geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0, position=position_dodge(0.8))+
  scale_colour_manual(name="Phenotype", values=parent_cols, labels=parent_labels)+
