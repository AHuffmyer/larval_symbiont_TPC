...
Conduct at biological process level. Filter for FDR adjusted p-value <0.05.  
```{r}
...
```
1549 up regulated and 1816 down regulated. We are keeping all contrasts in these lists and we will run a loop to conduct enrichment for each contrast.    
...
for (parent in parent_contrasts) {
    ...
    #run goseq using Wallenius method for all categories of GO terms 
    GO.wall<-goseq(pwf, gene2cat=GO.terms, test.cats=c("GO:BP"), method="Wallenius", use_genes_without_cat=FALSE)
...
```{r}
parent_contrasts<-c("NBvsB", "NBvsWT", "BvsWT")
...
Run a loop to filter enrichment for each contrast using rrvgo.   
```{r}
parent_contrasts<-c("BvsWT", "NBvsWT", "NBvsB")
...
length(unique(parent_go_results$GOterm))
```
There are now 68 enriched GO terms (3 were upregulated, same 3 down regulated in diff group). 
...
parent_go_plot_up <- parent_go_results %>% 
  ...
  ggplot(aes(x=contrast, y=term, colour=bh_adjust, size=percHits)) +
  facet_grid(ParentTerm ~ ., scales="free", labeller = label_wrap_gen(multi_line = TRUE), space="free")+
  geom_point()+
  scale_colour_gradientn(colours = colorspace::heat_hcl(7))+
  scale_size(name="Hits (%)", range = c(1, 5))+
...
parent_go_plot_down <- parent_go_results %>% 
  ...
  ggplot(aes(x=contrast, y=term, colour=bh_adjust, size=percHits)) +
  facet_grid(ParentTerm ~ ., scales="free", space="free", labeller = label_wrap_gen(multi_line = TRUE))+
  geom_point()+
  scale_colour_gradientn(colours = colorspace::heat_hcl(7))+
  scale_size(name="Hits (%)", range = c(1, 5))+
  theme_classic() +
...
parent_go_plot_parentA<-parent_go_results%>%
  ...
  #facet_grid(~ direction, scales="free_y", space="free", labeller = label_wrap_gen(width = 2, multi_line = TRUE))+
  geom_point(aes(size=percHits))+
  scale_size(name="Num. GO Terms", breaks=c(2,10,20,50,80), range = c(1, 5))+
  scale_colour_manual(values=c("blue", "red"))+
  scale_x_discrete(limits=c("BvsWT", "NBvsWT", "NBvsB"))+
...
ggsave(plot=parent_go_plot_parentA, "figures/rna_seq/functional_enrichment/parent_GO_parent.png", height=10, width=8, limitsize=FALSE)
```
...
ggsave(plot=interaction_deg_FC, "figures/rna_seq/functional_enrichment/interaction_DEG_foldchange.png", height=12, width=14)
```
...
```
37 up regulated and 65 down regulated. We are keeping all contrasts in these lists and we will run a loop to conduct enrichment for each contrast.    
...
Conduct at biological process level.   
```{r}
