...
Set colors and dpf values. 
```{r}
parent_names<-c("Bleached" = "Susceptible (C)", "Nonbleached" = "Resistant (C/D)", "Wildtype" = "Wildtype (C/D)")

df_cells<-df_cells%>%
  mutate(dpf=if_else(Date=="20230622", "3", 
                     if_else(Date=="20230623", "5", 
                     if_else(Date=="20230624", "6",
                     if_else(Date=="20230626", "8",
                     if_else(Date=="20230627", "9", NA
                     ))))))
...
```{r}
plot1<-df_cells %>%
  ggplot(aes(x = dpf, y = cells.larvae, color = Parent)) +
  labs(x = "DPF") +
  ylab(expression(paste("Symbiont cell density (cells larva" ^-1, ")")))+
  scale_color_manual(values=c("orange", "red4", "darkgray"), labels=parent_names)+
...
df_cells %>%
  ...
  ggplot(aes(x = dpf, y = mean, color = Parent)) +
...
```{r}
plot2<-size %>%
  ggplot(aes(x = dpf, y = volume, color = Parent)) +
  labs(x = "DPF",y = "Larval Volume") +
  scale_color_manual(values=c("orange", "red4", "darkgray"), labels=parent_names)+
  geom_boxplot(aes(group=interaction(Date,Parent)), position=position_dodge(0.8))+
  geom_point(position=position_jitterdodge(0.1))+
  #geom_smooth(aes(group=Parent), method="lm", se=FALSE)+
...
```{r}
plot3<-merged %>%
  ggplot(aes(x = dpf, y = cells.mm3, color = Parent)) +
  labs(x = "DPF") +
