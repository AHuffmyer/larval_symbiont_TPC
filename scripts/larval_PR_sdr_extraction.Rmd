...
```{r}
Run.Info<-read_csv("data/pr_sdr/PR_SDR_Run_Info.csv")%>%
...
metadata <- full_join(Sample.Info, Run.Info, by=c("Run", "Plate", "Date", "SDR", "Temperature"))
```
...
```{r}
# --- paths ---
input_dir  <- "data/pr_sdr/runs"
output_dir <- "output/pr_sdr/PhotosynthesisPlots"
...
readr::write_csv(summary_df, file.path("output/pr_sdr/photosynthesis_slope_summary_by_light_level.csv"))
```
...
readr::write_csv(temp_summary_plate, file.path("output/pr_sdr/mean_temperature_by_plate_and_light_interval.csv"))
```
