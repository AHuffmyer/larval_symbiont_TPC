...
lollipop_parent_pc1 <- lollipop_parent_pc1 %>%
  arrange(rank) %>%
  mutate(panel = ceiling(row_number() / (n()/2)))%>%
  mutate(pattern = if_else(panel=="1", "Higher in Susceptible (C)", 
                           if_else(panel=="2", "Higher in Susceptible (C) ", NA)))%>%
  mutate(pattern = if_else(loading>0, pattern, "Lower in Susceptible (C)"))

lolli_parent_pc1_plot<-lollipop_parent_pc1%>%
  
  ggplot(aes(x=loading, y=reorder(gene, -rank), color=class))+
  facet_wrap(~pattern, ncol = 3, scales = "free") +
  geom_point()+
  geom_segment(aes(x = 0, xend = loading, y = gene, yend = gene), linewidth = 0.8) +
  ylab("Lipid")+
  scale_color_manual(values = okabe_ito_19) +
...
ggsave(lolli_parent_pc1_plot, filename="figures/lipids/asca/parent_pc1_lipid_loadings.png", width=16, height=7)
```
...
lollipop_parent_pc2 <- lollipop_parent_pc2 %>%
  arrange(rank) %>%
  mutate(panel = ceiling(row_number() / (n()/2)))%>%
  mutate(pattern = if_else(loading>0, "Higher in wildtype", "Lower in wildtype"))
...
ggsave(lolli_parent_pc2, filename="figures/lipids/asca/lion/parent_pc2_terms.png", width=7, height=5)
```
