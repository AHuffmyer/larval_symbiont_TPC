...
Use the dark 26 for respiration
Use the light 26 for photosynthesis 
Use the dark 26 for respiration under different temperatures 

Don't use the RLC for this 

Use O2 Fm; Use QY (only for dark periods at the end)

Subset the last 5 min of each section for all metrics 

Use NPQ for all intervals



O2 - Fo = measured at steady state light before saturating pulse for photophys measurements. Dont use this one. 
O2 - Fm = measured after saturating pulse. Use this metric for O2, its consistent light. Lower flourescence = more O2; Higher fluorescence = less O2. 02 quenches the fluorescence. 

QY (quantum yield) = Only use from dark phases. 

NPQ = use this one for all phases 
